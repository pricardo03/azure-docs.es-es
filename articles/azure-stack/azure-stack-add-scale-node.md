...
2. Configure la dirección IP correcta en el controlador de administración de placa base (BMC) y aplique toda la configuración de BIOS según la documentación proporcionada por el OEM.
3. Aplique la base de referencia del firmware actual a todos los componentes mediante las herramientas proporcionadas por el fabricante del hardware que se ejecutan en el HLH.
4. Ejecute la operación Agregar nodo en el portal de administración de Azure Stack.
5. Valide que la operación Agregar nodo se realiza correctamente. Para ello, compruebe el [**estado** de la unidad de escalado](#monitor-add-node-operations). 

## <a name="add-the-node"></a>Incorporación del nodo 
Puede usar la consola de administrador o PowerShell para agregar nuevos nodos. La operación Agregar nodo primero agrega el nuevo nodo de unidad de escalado como capacidad de proceso disponible y luego extiende la capacidad de almacenamiento automáticamente. La capacidad se expande automáticamente porque Azure Stack es un sistema hiperconvergido donde *proceso* y *almacenamiento* escalan juntos.

### <a name="use-the-admin-console"></a>Uso de la consola de administración
1. Inicie sesión en el portal de administración de Azure Stack como operador de Azure Stack.
2. Vaya a **Region management** (Administración de regiones) > **Scale units** (Unidades de escalado) y seleccione la unidad de escalado para la que desea expandir la capacidad.  
   ![Seleccionar unidad de escalado](media/azure-stack-add-scale-node/select-node1.png)

   ![Vista de los detalles de la unidad de escalado](media/azure-stack-add-scale-node/select-node2.png)
 
3. Se necesitan los siguientes parámetros:  
   - **DIRECCIÓN IP de BMC** para cada nodo de unidad de escalado que agrega. (Una dirección IP por línea). ![Agregar nodo](media/azure-stack-add-scale-node/add-node.png)

### <a name="use-powershell"></a>Uso de PowerShell

Use el cmdlet **New-AzsScaleUnitNodeObject** para agregar uno o varios nodos.  
...
## <a name="monitor-add-node-operations"></a>Supervisión de las operaciones Agregar nodo 
