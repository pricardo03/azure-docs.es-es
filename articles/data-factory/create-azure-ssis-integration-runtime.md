...
- **Servidor lógico o Instancia administrada de Azure SQL Database**. Si aún no tiene un servidor de bases de datos, cree uno en Azure Portal antes de empezar. Este servidor va a hospedar la base de datos del catálogo de SSIS (SSISDB). Le recomendamos que cree el servidor de bases de datos en la misma región de Azure que el entorno de ejecución de integración. Esta configuración permite que el entorno de ejecución de integración escriba registros de ejecución en SSISDB sin traspasar regiones de Azure. En función del servidor de bases de datos seleccionado, la SSISDB puede crearse en su nombre como base de datos única, como parte de un grupo elástico o en una instancia administrada y estar accesible desde la red pública o mediante la unión a una red virtual. Para una lista de los planes de tarifa admitidos para Azure SQL Database, consulte [Límites de recursos de SQL Database](../sql-database/sql-database-resource-limits.md). 

    Asegúrese de que el servidor lógico o Instancia administrada de Azure SQL Database no tienen ya un catálogo de SSIS (base de datos SSIDB). El aprovisionamiento de entornos de ejecución de SSIS en Azure no admite el uso de un catálogo de SSIS existente. 
...
- **Red virtual de Azure Resource Manager o clásica (opcional)**. Debe tener una instancia de Azure Virtual Network si se cumple al menos una de las siguientes condiciones: 
    - Hospeda la base de datos de catálogo de SSIS en Azure SQL Database con puntos de conexión de servicio de red virtual / Instancia administrada de Azure SQL Database que está dentro de una red virtual. 
...
En la siguiente tabla se comparan determinadas características del servidor lógico de SQL Database e Instancia administrada en su relación con el entorno de ejecución de integración de SSIS en Azure:

| Característica | Servidor lógico de SQL Database| Instancia administrada de SQL Database |
|---------|--------------|------------------|
| **Programación** | El agente SQL Server no está disponible.<br/><br/>Consulte [Schedule a package as part of an Azure Data Factory pipeline](/sql/integration-services/lift-shift/ssis-azure-schedule-packages#activity) (Programación de un paquete como parte de una canalización de Azure Data Factory).| El agente SQL Server está disponible. |
...
| **Transacciones distribuidas** | No se admiten las transacciones del Coordinador de transacciones distribuidas de Microsoft (MSDTC). Si los paquetes usan MSDTC para coordinar las transacciones distribuidas, puede implementar una solución temporal mediante transacciones elásticas para SQL Database. En este momento, SSIS no tiene compatibilidad integrada con transacciones elásticas. Para usar transacciones elásticas en el paquete de SSIS, tiene que escribir código personalizado de ADO.NET en una tarea de script. Esta tarea de script debe incluir el principio y el final de la transacción y todas las acciones que tienen que tener lugar dentro de la transacción.<br/><br/>Para más información sobre cómo codificar transacciones elásticas, consulte [Elastic Database Transactions with Azure SQL Database](https://azure.microsoft.com/blog/elastic-database-transactions-with-azure-sql-database/) (Transacciones de base de datos elásticas con Azure SQL Database). Para más información sobre las transacciones elásticas en general, consulte [Transacciones distribuidas entre bases de datos en la nube](../sql-database/sql-database-elastic-transactions-overview.md). | No compatible. |
| | | |
...
1. En la página **SQL Settings** (Configuración de SQL), realice los pasos siguientes: 
   ...
   ![Configuración de SQL](./media/tutorial-create-azure-ssis-runtime-portal/sql-settings.png)
    ...
    d. En la casilla **Use AAD authentication...** (Usar autenticación de AAD...), seleccione el método de autenticación del servidor de bases de datos para hospedar SSISDB: SQL o Azure Active Directory (AAD) con la identidad administrada de Azure Data Factory para los recursos de Azure. Si se activa, debe agregar la instancia de MSI de Data Factory a un grupo de AAD con permisos de acceso al servidor de base de datos; para ello, consulte [Habilitación de la autenticación de AAD para la instancia de IR de SSIS de Azure](https://docs.microsoft.com/azure/data-factory/enable-aad-authentication-azure-ssis-ir). 
...
1. En la casilla **Select a virtual network...**  (Seleccionar una red virtual...), seleccione si desea unir el entorno de ejecución de integración a una red virtual. Active la casilla si usa Azure SQL Database con puntos de conexión de servicio de red virtual / Instancia administrada de Azure SQL Database para hospedar SSISDB o si necesita acceder a los datos locales; es decir, tiene orígenes o destinos de datos locales en los paquetes de SSIS. Puede consultar [Unión de una instancia de Integration Runtime de SSIS de Azure a una red virtual](https://docs.microsoft.com/azure/data-factory/join-azure-ssis-integration-runtime-virtual-network). Si activa la casilla, realice los pasos siguientes: 
...
1. Haga clic en **VNet Validation** (Validación de red virtual) y, si resulta correcta, haga clic en **Finish** (Finalizar) para comenzar la creación de la instancia de Integration Runtime de SSIS de Azure. 
...
1. En la ventana **Connections**, cambie a **Integration Runtime** si es necesario. Haga clic en **Refresh** (Actualizar) para actualizar el estado. 

   ![Estado de la creación](./media/tutorial-create-azure-ssis-runtime-portal/azure-ssis-ir-creation-status.png)

1. Use los vínculos de la columna **Acciones** para supervisar, detener, iniciar, editar o eliminar el entorno de ejecución de integración. Utilice el último vínculo para ver el código JSON de Integration Runtime. Los botones de edición y eliminación se habilitan únicamente cuando la instancia de IR se detiene. 
...
1. Para crear una instancia de Integration Runtime de SSIS de Azure, haga clic en **New** (Nuevo) tal como se muestra en la imagen. 
1. En la ventana de configuración de Integration Runtime, seleccione **Lift-and-shift existing SSIS packages to execute in Azure** (Migrar los paquetes de SSIS existentes mediante lift-and-shift para la ejecución en Azure) y haga clic en **Next** (Siguiente). 
...
1. Consulte la sección [Aprovisionamiento de Integration Runtime de SSIS de Azure](#provision-an-azure-ssis-integration-runtime) para el resto de pasos de configuración de IR de SSIS de Azure. 
...
Si usa la autenticación de Azure Active Directory (AAD) con la identidad administrada de Azure Data Factory para que los recursos de Azure se conecten al servidor de bases de datos, puede omitir el parámetro CatalogAdminCredential, pero debe agregar la instancia de MSI de Data Factory a un grupo de AAD que cuente con permisos de acceso al servidor de bases de datos; para ello, consulte [Enable AAD authentication for Azure-SSIS IR](https://docs.microsoft.com/azure/data-factory/enable-aad-authentication-azure-ssis-ir) (Habilitar la autenticación de AAD para Azure-SSIS IR). En caso contrario, no puede omitirlo y debe pasar un objeto válido formado a partir de su nombre de usuario y contraseña de administrador del servidor para la autenticación de SQL.
...
1. Para iniciar la instancia de IR de SSIS de Azure, ejecute el comando Start-AzureRmDataFactoryV2IntegrationRuntime: 
