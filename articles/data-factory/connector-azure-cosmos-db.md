---
title: Copia y transformación de datos en Azure Cosmos DB (SQL API) mediante Data Factory
description: Obtenga información sobre cómo copiar datos con Azure Cosmos DB (SQL API) como origen y destino y cómo transformar datos en Azure Cosmos DB (SQL API) mediante Data Factory.
...
| Propiedad | DESCRIPCIÓN | Obligatorio |
|:--- |:--- |:--- |
| Tipo | La propiedad **type** del conjunto de datos debe establecerse en **CosmosDbSqlApiCollection**. |Sí |
...
Data Factory respeta la asignación especificada en la actividad. Si una fila no contiene un valor para una columna, se proporciona un valor nulo para la columna.

Si no especifica una asignación, el servicio Data Factory deduce el esquema mediante la primera fila de los datos. Si la primera fila no contiene el esquema completo, algunas columnas se pueden perder en el resultado de la operación de la actividad.
...
| Propiedad | DESCRIPCIÓN | Obligatorio |
|:--- |:--- |:--- |
| Tipo | La propiedad **type1** del origen de la actividad de copia debe establecerse en **CosmosDbSqlApiSource**. |Sí |
...
| preferredRegions | Lista preferida de regiones a las que se conectará cuando recupere los datos desde Cosmos DB. | Sin |
| pageSize | Número de documentos por página del resultado de la consulta. El valor predeterminado es "-1", que significa el uso del tamaño de página dinámica del servicio hasta 1000. | Sin |
...
| Propiedad | DESCRIPCIÓN | Obligatorio |
|:--- |:--- |:--- |
| Tipo | La propiedad **type** del receptor de la actividad de copia tiene que establecerse en **CosmosDbSqlApiSink**. |Sí |
