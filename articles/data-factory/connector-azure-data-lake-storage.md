...
>[!IMPORTANT]
>Si usa PolyBase para cargar los datos de ADLS Gen2 en SQL DW, cuando mediante ADLS Gen2 administrado la autenticación de la identidad, asegúrese de que también sigue los pasos #1 y #2 en [esta guía](../sql-database/sql-database-vnet-service-endpoint-rule-overview.md#impact-of-using-vnet-service-endpoints-with-azure-storage) para registrar el servidor de base de datos SQL con Azure Active Directory Directory (AAD) y asignar roles de RBAC de colaborador de datos de Blob de almacenamiento para el servidor de base de datos SQL; el resto se tratará de ADF. Si su Gen2 ADLS se configura con el punto de conexión de servicio de red virtual, para usar PolyBase para cargar datos de él, debe usar autenticación de identidad administrada.
