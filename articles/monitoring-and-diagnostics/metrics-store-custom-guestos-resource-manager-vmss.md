...
Mediante el uso de la [extensión Windows Azure Diagnostics (WAD)](azure-diagnostics.md) de Azure Monitor, puede recopilar métricas y registros del sistema operativo invitado (SO invitado) que se ejecuta como parte de una máquina virtual, servicio en la nube, o clúster de Azure Service Fabric. La extensión puede enviar datos de telemetría a muchas ubicaciones diferentes que aparecen en el artículo vinculado anteriormente.  
...
- La suscripción debe estar registrada en [Microsoft.Insights](https://docs.microsoft.com/en-us/azure/azure-resource-manager/resource-manager-supported-services#portal). 
...
1. En la página **Monitor**, seleccione **Métricas**. 
   ...
   ![Página Métricas del Monitor](media/metrics-store-custom-guestos-resource-manager-vmss/metrics.png) 
