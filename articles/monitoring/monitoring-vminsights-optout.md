...
Si antes de incorporarse a Azure Monitor para VM, no estaba [recopilando los contadores de rendimiento habilitados](monitoring-vminsights-onboard.md?toc=/azure/azure-monitor/toc.json#performance-counters-enabled) para las máquinas virtuales basadas en Windows o Linux en el área de trabajo, necesita deshabilitar esas reglas siguiendo los pasos descritos [aquí](../log-analytics/log-analytics-data-sources-performance-counters.md?toc=/azure/azure-monitor/toc.json#configuring-performance-counters) para Windows y para Linux.
