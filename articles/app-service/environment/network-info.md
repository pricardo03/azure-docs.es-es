...
Si tiene un ASE de ILB, la dirección de la dirección del ILB es el punto de conexión para HTTP/S, FTP/S, implementación web y depuración remota.

## <a name="ase-subnet-size"></a>Tamaño de la subred de ASE ##

El tamaño de la subred que se utiliza para hospedar una instancia de ASE no se puede modificar una vez que dicha instancia de ASE se ha implementado.  La instancia de ASE utiliza una dirección para cada rol de la infraestructura, así como para cada instancia del plan de App Service en entorno aislado.  Además, hay cinco direcciones que usa redes de Azure para cada subred que se crea.  Las instancias de ASE que no tengan ningún plan de App Service utilizarán 12 direcciones antes de que se cree una aplicación.  Si es un ASE de ILB, a continuación, utilizará 13 direcciones antes de crear una aplicación en dicho ASE. A medida que escale horizontalmente el ASE, los roles de infraestructura se agregan a cada múltiplo de 15 y 20 de las instancias del plan de App Service.
...
Al escalar o reducir verticalmente, se agregan nuevos roles del tamaño adecuado y, a continuación, las cargas de trabajo se migran del tamaño actual al tamaño de destino. Original máquinas virtuales se quitan solo después de que se han migrado las cargas de trabajo. Si tuviera un ASE con 100 instancias ASP, habrá un período donde necesite duplique el número de máquinas virtuales.  Es por esta razón que se recomienda el uso de un "/24" para acomodar cualquier cambio que pueda necesitar.  
...
Dentro de la subred de ASE, hay muchos puertos usados para la comunicación de un componente interno y pueden cambiar. Esto requiere que todos los puertos de la subred de ASE sean accesibles desde la subred de ASE. 

Para la comunicación entre el equilibrador de carga de Azure y la subred de ASE, los puertos mínimos que deben abrirse son 454, 455 y 16001. El puerto 16001 se usa para mantener activo el tráfico entre el equilibrador de carga y el ASE. Si usas un ASE de ILB, a continuación, puede bloquear el tráfico solo en los puertos 454, 455 y 16001 puertos.  Si usas un ASE externo, deberá tener en cuenta los puertos de acceso de aplicación normal.  Si usa direcciones asignadas a las aplicaciones, deberá abrirlo a todos los puertos.  Cuando se asigna una dirección a una aplicación específica, el equilibrador de carga usará puertos no conocidos de antemano para enviar tráfico HTTP y HTTPS al ASE.

Si usa direcciones IP asignadas a las aplicaciones, deberá permitir el tráfico de las direcciones IP asignadas a las aplicaciones para la subred de ASE.
...
El ASE se comunica horizontal direcciones accesible de internet en los siguientes puertos:

| Port | Usos |
|-----|------|
| 53 | DNS |
| 123 | NTP |
| 80/443 | CRL, las actualizaciones de Windows, las dependencias de Linux, servicios de Azure |
| 1433 | Azure SQL | 
| 12000 | Supervisión |
...
Si la red virtual se configura con un servidor DNS definido por el cliente, las cargas de trabajo del inquilino lo utilizan. La instancia de ASE utiliza DNS de Azure para fines de administración. Si la red virtual se configura con un servidor DNS seleccionados por el cliente, el servidor DNS debe ser accesible desde la subred que contiene el ASE.

Para probar la resolución DNS de su aplicación web, puede usar el comando de consola *nameresolver*. Vaya a la ventana de depuración en el sitio de scm para su aplicación o vaya a la aplicación en el portal y seleccione la consola. Desde el símbolo del shell puede emitir el comando *nameresolver* junto con el nombre DNS que desea buscar. El resultado que se obtiene es el mismo que el que obtendría la aplicación al hacer la misma búsqueda. Si usa nslookup, deberá realizar una búsqueda con Azure DNS en su lugar.
...
Cuando se usa un ASE de ILB, el sitio SCM no está accesible desde fuera de la red virtual. Algunas funcionalidades no funcionará desde el portal de la aplicación ya que requieren acceso al sitio SCM de una aplicación. Puede conectarse al sitio de SCM directamente en lugar de usar el portal. 

Si su ASE de ILB es el nombre de dominio *contoso.appserviceenvironnment.net* y es el nombre de la aplicación *testapp*, la aplicación está accesible en *testapp.contoso.appserviceenvironment.net*. El sitio SCM que lo acompaña está accesible en *testapp.scm.contoso.appserviceenvironment.net*.
...
Todas estas direcciones IP son visibles en el portal de Azure desde la interfaz de usuario de ASE. Si tiene un ASE de ILB, aparece la dirección IP para el ILB.
...
Las entradas necesarias en un NSG, para un ASE funcione, son permitir el tráfico:

**Inbound**
* desde la dirección IP de la etiqueta AppServiceManagement en puertos 454,455 de servicio
* el equilibrador de carga en el puerto 16001
* en la subred de ASE a la subred de ASE en todos los puertos

**Outbound**
* para todas las direcciones IP en el puerto 123
* para todas las direcciones IP en los puertos 80, 443
* a la dirección IP AzureSQL en los puertos 1433 de la etiqueta de servicio
* para todas las direcciones IP en el puerto 12000
* a la subred de ASE en todos los puertos

El puerto DNS no es necesario a medida que el tráfico DNS no se ve afectado por las reglas de NSG. Estos puertos no incluyen los puertos que requieren las aplicaciones para utilizar correctamente. Los puertos de acceso de aplicación normales son:
