---
title: Cifrado de datos de un solo servidor de Azure Database for PostgreSQL con la clave administrada por el cliente
description: El cifrado de datos de un solo servidor de Azure Database for PostgreSQL con una clave administrada por el cliente habilita Bring Your Own Key (BYOK) para la protección de datos en reposo y permite a las organizaciones implementar la separación de tareas en la administración de claves y datos.
author: kummanish
ms.author: manishku
...
ms.locfileid: "76028616"
---
# <a name="azure-database-for-postgresql-single-server-data-encryption-with-customer-managed-key"></a>Cifrado de datos de un solo servidor de Azure Database for PostgreSQL con la clave administrada por el cliente

> [!NOTE]
> En este momento, debe solicitar acceso para poder usar esta funcionalidad. Para ello, póngase en contacto con AskAzureDBforPostgreSQL@service.microsoft.com.
...
* Mayor transparencia y control granular y administración para la clave de cifrado 
...
![Introducción a Bring Your Own Key](media/concepts-data-access-and-security-data-encryption/postgresql-data-encryption-overview.png)

Para que el servidor PostgreSQL pueda usar las claves administradas por el cliente almacenadas en Azure Key Vault para cifrar las DEK, el administrador de Key Vault debe conceder los siguientes derechos de acceso al servidor mediante su identidad única:
...
* Conceda al servidor único de Azure Database for PostgreSQL acceso a Key Vault con los permisos **get, wrapKey, unwrapKey** mediante su identidad administrada única. Cuando se usa Azure Portal, la identificación única se crea automáticamente cuando se habilita el cifrado de datos en el servidor único de PostgreSQL. Consulte [Configuración del cifrado de datos para un solo servidor de PostgreSQL](howto-data-encryption-portal.md) para obtener instrucciones detalladas al usar Azure Portal.
...
### <a name="requirements-for-configuring-customer-managed-key"></a>Requisitos de configuración de clave administrada por el cliente
...
Obtenga información sobre cómo [Configurar el cifrado de datos con la clave administrada por el cliente para el servidor único de Azure Database for PostgreSQL mediante Azure Portal](howto-data-encryption-portal.md).
