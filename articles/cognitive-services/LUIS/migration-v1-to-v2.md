...
Las API de clave de suscripción están en desuso y devuelven el error 410 "Desaparecido".
...
Las [claves de suscripción](luis-how-to-azure-subscription.md) de Azure se generan en Azure Portal. Asigne la clave a una aplicación de LUIS en la página **[Publicar](manage-keys.md)**. No tiene que conocer el valor real de la clave. LUIS usa el nombre de la suscripción para realizar la asignación. 
...
LUIS sugiere expresiones a partir de [expresiones de punto de conexión](label-suggested-utterances.md) existentes que podrían mejorar el modelo. En la versión anterior, esto se llamaba **sample**. En la nueva versión, el nombre ha cambiado de sample a **suggest**. Esto se denomina **[Review endpoint utterances](https://docs.microsoft.com/azure/cognitive-services/LUIS/label-suggested-utterances)** (Revisar las expresiones de punto de conexión) en el sitio web de LUIS.
...
Las [entidades creadas previamente](Pre-builtEntities.md) han cambiado. Asegúrese de que está usando las entidades creadas previamente de la v2. Esto incluye el uso de [datetimeV2](pre-builtentities.md?#use-a-prebuilt-datetimev2-entity), en lugar de datetime. 
