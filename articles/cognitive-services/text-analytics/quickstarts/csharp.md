...
En la clase `Program` de la aplicación, cree variables para el punto de conexión y la clave de suscripción de Azure del recurso. En un constructor estático, obtenga estos valores de las variables de entorno `TEXT_ANALYTICS_SUBSCRIPTION_KEY` y `TEXT_ANALYTICS_ENDPOINT`. Si creó estas variables de entorno después de haber comenzado a editar la aplicación, deberá cerrar y volver a abrir el editor, el IDE o el shell que usa para acceder a las variables.
...
En el método `Main` de la aplicación, cree credenciales para acceder al punto de conexión de Text Analytics.  Más adelante definirá los métodos a los que llama el método `Main`.
