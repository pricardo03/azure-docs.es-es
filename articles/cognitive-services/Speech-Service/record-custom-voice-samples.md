...
Hay cuatro roles básicos en un proyecto de grabación de voz personalizada.
...
Un usuario individual puede desempeñar más de un rol. En esta guía se da por hecho que desempeñará principalmente el rol de director y que contratará un actor de voz y un ingeniero de grabación. En caso de que quiera realizar las grabaciones usted mismo, se incluye alguna información sobre el rol de ingeniero de grabación.

## <a name="choosing-voice-talent"></a>Selección del actor de voz
...
Una persona puede tener, por ejemplo, una personalidad naturalmente animada. Así que su voz puede llevar una nota de optimismo incluso aunque hable de forma neutra. Sin embargo, tal rasgo de personalidad debe ser sutil y uniforme. Escuche las lecturas de voces existentes para hacerse una idea de lo que busca.
...
## <a name="creating-a-script"></a>Creación de un guion
...
Puede escribir el guion en Microsoft Word. El guion se usará durante la sesión de grabación, así que puede prepararlo de forma que sea fácil trabajar con él. Cree aparte el archivo de texto necesario en el portal Custom Voice.
...
> [!NOTE]
> La mayoría de los estudios graban en segmentos cortos conocidos como *cortes.* Cada corte contiene normalmente de 10 a 24 enunciados. Con solo anotar el número de corte se podrá encontrar un enunciado más tarde. Si graba en un estudio que prefiere realizar grabaciones más largas, querrá anotar entonces el código de tiempo. El estudio tendrá una gran pantalla de tiempo.
...
### <a name="doing-it-yourself"></a>Hacerlo uno mismo
...
También puede usar un micrófono analógico. Muchas de las empresas de alquiler ofrecen micrófonos "vintage" reconocidos por sus personajes de voz. Tenga en cuenta que el equipo analógico profesional usa conectores XLR balanceados, en lugar del conector de 1/4" usado en los equipos de consumo. Si se decide por el analógico, también necesitará un preamplificador y una interfaz de audio de equipo informático con estos conectores.

Coloque el micrófono en un pie o soporte tipo jirafa e instale un filtro contra chasquidos para eliminar el ruido de los sonidos oclusivos, como la "p" y la "b". Algunos micrófonos incorporan un montaje de suspensión que los aísla de las vibraciones del pie, lo que es útil.
...
Escuche con atención la grabación de silencio en la cabina, averigüe de dónde proceden los ruidos y elimine la causa. Las fuentes habituales de ruido son rendijas de ventilación, balastros de tubos fluorescentes, el tráfico de carreteras cercanas y los ventiladores de los equipos (incluso los portátiles pueden tener ventiladores). Los micrófonos y cables pueden captar el ruido eléctrico de los cables cercanos de CA, normalmente un zumbido o murmullo.
...
Grabe directamente en el equipo informático con una interfaz de audio de alta calidad o un puerto USB, según el micrófono que vaya a usar. Si es analógico, use lo básico: micrófono, preamplificador, interfaz de audio, equipo informático. Tanto [Avid Pro Tools](http://www.avid.com/en/pro-tools) como [Adobe Audition](https://www.adobe.com/products/audition.html) se pueden usar con licencia mensual por un módico precio. Si su presupuesto es muy reducido, pruebe la aplicación gratuita [Audacity](https://www.audacityteam.org/).

Grabe con sonido monofónico de 16 bits a 44,1 KHz (calidad de CD) o mejor. El más moderno actualmente es 24 bits a 48 KHz, si su equipo lo admite. La calidad del audio se reduce a 16 bits a 16 KHz antes de enviarlo al portal Custom Voice. Aun así, vale la pena tener una grabación original de alta calidad en caso de que sea necesario realizar modificaciones.

Lo ideal es tener distintas personas en los roles de director, ingeniero y actor. No intente hacerlos todos usted mismo. En caso de emergencia, el director y el ingeniero pueden ser una persona.
...
El actor *no* debe agregar pausas claras entre las palabras. La frase debe fluir naturalmente, aunque suene un poco formal. Esta distinción fina puede requerir práctica para que salga bien.
...
El archivo de ajuste es especialmente importante al reanudar la grabación tras un descanso, u otro día. Lo reproducirá varias veces para el actor y le hará repetirlo cada vez hasta que la coincidencia sea perfecta.

Entrene a su actor para respirar profundamente y hacer una pausa durante un momento antes de cada enunciado. Grabe un par de segundos de silencio entre enunciados. Las palabras se deben pronunciar siempre igual cada vez que aparezcan, por ejemplo: "Miami" se puede pronunciar "Mayami" o "Miami".
...
Los estudios de grabación modernos funcionan en equipos informáticos. Al final de la sesión, recibirá uno o varios archivos de audio, no una cinta. Estos archivos tendrán probablemente el formato WAV o AIFF en calidad de CD (16 bits a 44,1 KHz) o mejor. 24 bits a 48 KHz es lo habitual y deseable. Por lo general, no se necesitan velocidades de muestreo más altas, como 96 KHz.

El portal Custom Voice exige que cada enunciado se proporcione en su propio archivo. Los archivos de audio entregados por el estudio contienen varios enunciados. Así que la principal tarea de posproducción es separar las grabaciones y prepararlas para su envío. El ingeniero de grabación puede haber colocado marcadores en el archivo (o proporcionado una lista de pistas aparte) para indicar dónde comienza cada enunciado.

Use sus notas para encontrar exactamente los cortes que desee y luego use una utilidad de edición de sonido, como [Avid Pro Tools](http://www.avid.com/en/pro-tools), [Adobe Audition](https://www.adobe.com/products/audition.html), o la aplicación gratuita [Audacity](https://www.audacityteam.org/) para copiar cada enunciado en un nuevo archivo.

Deje solamente unos 0,2 segundos de silencio al principio y al final de cada clip, excepto el primero. Ese archivo debe empezar con cinco segundos completos de silencio. No utilice el editor de audio para "poner a cero" las partes de silencio del archivo. Incluir el "tono de la sala" ayudará a los algoritmos de Custom Voice a compensar los ruidos de fondo que hayan podido quedar.
...
Convierta cada archivo de 16 bits y una velocidad de muestreo de 16 KHz antes de guardar y, si graba la charla del estudio, quite el segundo canal. Guarde cada archivo en formato WAV y nombre los archivos con el número de enunciado del guion.

Por último, cree la *transcripción* que asocia cada archivo WAV con una versión de texto del enunciado correspondiente. En [Crear fuentes de voz personalizada](how-to-customize-voice-font.md) se incluye información detallada del formato requerido. Puede copiar el texto directamente del guion. A continuación, cree un archivo ZIP de los archivos WAV y de la transcripción de texto.
...
Está listo para cargar las grabaciones y crear su voz personalizada.
