...
ms.locfileid: "47046704"
---
# New-AzureRmFrontDoorRoutingRuleObject
Crear un objeto PSRoutingRuleObject para la creación de Front Door
...
### Ejemplo 1: Crear un objeto PSRoutingRuleObject para la creación de Front Door
```powershell
...
### -AcceptedProtocol
Esquemas de protocolos que deben coincidir con esta regla.
...
### -BackendPoolName
Identificador del recurso de BackendPool al que se enruta esta regla
...
### -CustomForwardingPath
La ruta de acceso personalizada usada para reescribir las rutas de acceso del recurso con las que coincide esta regla.
Déjela en blanco para usar la ruta de acceso entrante.
...
### -DefaultProfile
Las credenciales, la cuenta, el inquilino y la suscripción usados para la comunicación con Azure.
...
### -DynamicCompression
Si desea habilitar la compresión dinámica para el contenido almacenado en caché cuando el almacenamiento en caché está habilitado.
...
### -EnableCaching
Si desea habilitar el almacenamiento en caché para esta ruta. El valor predeterminado es False
...
### -EnabledState
Si desea habilitar el uso de esta regla.
El valor predeterminado es Enabled
...
### -ForwardingProtocol
El protocolo que usará esta regla al reenviar el tráfico al valor predeterminado de back-ends es MatchRequest.
...
### -FrontDoorName
El nombre de Front Door al que pertenece esta regla de enrutamiento.
...
Los nombres de los puntos de conexión de front-end asociados con esta regla
...
### -PatternToMatch
Los patrones de ruta de la regla. No debe tener ningún *, excepto, posiblemente, después de la última / al final de la ruta de acceso. El valor predeterminado es /*
...
El tratamiento de los términos de la consulta de URL al formar la clave de caché.
El valor predeterminado es StripAll
...
### -ResourceGroupName
El nombre del grupo de recursos donde se creará RoutingRule.
