---
title: Uso del módulo de Azure Database Migration Service en Microsoft Azure PowerShell para migrar instancias locales de SQL Server a una Instancia administrada de Azure SQL DB | Microsoft Docs
description: Aprenda a migrar de SQL Server local a una Instancia administrada de Azure SQL DB mediante Azure PowerShell.
...
En este artículo, migrará la base de datos **Adventureworks2012** restaurada en una instancia local de SQL Server 2005 o superior a una instancia de Azure SQL Database mediante Microsoft Azure PowerShell. Puede migrar bases de datos desde una instancia de SQL Server local a Azure SQL Database mediante el módulo `AzureRM.DataMigration` en Microsoft Azure PowerShell.
...
- Haber completado la evaluación de su base de datos local y la migración de esquemas con Data Migration Assistant, tal y como se describe en el artículo [Performing a SQL Server migration assessment](https://docs.microsoft.com/sql/dma/dma-assesssqlonprem) (Realización de una evaluación de migración de SQL Server)
...
Cree una lista de objetos `AzureRmDataMigrationDatabaseInfo` que especifique las bases de datos como parte del proyecto de Azure Database Migration que puede proporcionarse como parámetro para la creación del proyecto. Puede usarse el cmdlet `New-AzureRmDataMigrationDatabaseInfo` para crear AzureRmDataMigrationDatabaseInfo. 

En el siguiente ejemplo se crea el proyecto `AzureRmDataMigrationDatabaseInfo` en la base de datos **AdventureWorks** y se agrega a la lista que se va a proporcionar como parámetro para la creación del proyecto.
...
Cree ahora el objeto FileShare que representa el recurso compartido de red SMB local en el que Azure Database Migration Service puede realizar las copias de seguridad de la base de datos de origen con el cmdlet New-AzureRmDmsFileShare.
...
El paso siguiente consiste en seleccionar las bases de datos de origen y destino mediante el cmdlet New-AzureRmDmsSelectedDB, tal y como se muestra en el siguiente ejemplo:
...
Cree la lista de inicios de sesión que se va a migrar tal y como se muestra en el ejemplo siguiente: tenga en cuenta que DMS solo admite actualmente la migración de inicios de sesión SQL. 
...
- *SourceConnection*. Objeto AzureRmDmsConnInfo que representa la conexión de origen de SQL Server.
- *TargetConnection*. Objeto AzureRmDmsConnInfo que representa la conexión de la Instancia administrada de destino de Azure SQL Database.
...
- *SelectedDatabase*. Objeto AzureRmDataMigrationSelectedDB que representa la asignación de base de datos de origen y de destino.
