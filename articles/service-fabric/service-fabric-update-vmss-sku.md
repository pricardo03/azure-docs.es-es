...
En el ejemplo siguiente, se utiliza la CLI de Azure Service Fabric para implementar la plantilla actualizada de Resource Manager *template.json* utilizando un grupo de recursos llamado *myResourceGroup*:
...
```
#### <a name="azure-cli"></a>Azure CLI
En el ejemplo siguiente, se utiliza la CLI de Azure Service Fabric CLI para deshabilitar la instancia del nodo *NTvm1_0* del antiguo conjunto de escalado de máquinas virtuales llamado *NTvm1*:
```CLI
sfctl node disable --node-name "_NTvm1_0" --deactivation-intent RemoveNode
```
2. Elimine todo el conjunto de escalado. Espere a que el estado de aprovisionamiento del conjunto de escalado sea Correcto
#### <a name="azure-powershell"></a>Azure PowerShell
...
```
#### <a name="azure-cli"></a>Azure CLI
En el ejemplo siguiente, se usa la CLI de Azure Service Fabric para quitar todo el conjunto de escalado *NTvm1* del grupo de recursos *myResourceGroup*:
...
En el ejemplo siguiente, se usa la CLI de Azure Service Fabric para quitar el balanceador de carga *LB-myCluster-NTvm1*, que está relacionado con el antiguo conjunto de escalado del grupo de recursos *myResourceGroup*:
...
En el ejemplo siguiente, se usa la CLI de Azure Service Fabric para obtener la configuración de DNS de una dirección IP pública llamada *LBIP-myCluster-NTvm1* y quitar la dirección IP:
...
En el ejemplo siguiente, se usa la CLI de Azure Service Fabric para actualizar la configuración de DNS de una dirección IP pública llamada *LBIP-myCluster-NTvm3* con la configuración de DNS de una dirección IP pública antigua obtenida en el paso anterior:
...
El comando siguiente utiliza la CLI de Azure Service Fabric para notificar a Service Fabric que se ha quitado el nodo *NTvm1_0*:
