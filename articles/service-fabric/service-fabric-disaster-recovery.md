...
Service Fabric tiene el concepto de los nodos raíz. Se trata de nodos que conservan la disponibilidad del clúster subyacente. Estos nodos ayudan a garantizar que el clúster permanece activo, al establecer concesiones con otros nodos y servir de elemento diferenciador durante ciertos tipos de errores de red. Si errores aleatorios eliminan la mayoría de los nodos raíz en el clúster y no se vuelvan a, el anillo de federación del clúster se contrae como has perdido el quórum del nodo de valor de inicialización y el clúster produce un error. En Azure, el proveedor de recursos de Service Fabric administra las configuraciones de clúster de Service Fabric y, de forma predeterminada, distribuye los nodos raíz en dominios de error y actualización del tipo de nodo principal; Si el tipo de nodo principal está marcada como la durabilidad Silver o Gold, al quitar un nodo de valor de inicialización, mediante el escalado en el tipo de nodo principal o quitar manualmente un nodo raíz, el clúster intentará promover otro nodo de valor de inicialización no desde el tipo de nodo principal disponible la capacidad y se producirá un error si tiene capacidad disponible menos que el clúster de nivel de confiabilidad se requiere para el tipo de nodo principal.
