---
title: Creación de un plano técnico de Azure Blueprint con la API REST
...
Si no está familiarizado con la API REST, comience por revisar la [referencia de API REST de Azure](/rest/api/azure/) para obtener una descripción general de la API REST, específicamente la URI de solicitud y cuerpo de la solicitud. En este artículo se utilizan estos conceptos para proporcionar instrucciones para trabajar con planos técnicos de Azure Blueprint y da por supuesto un conocimiento práctico de ellos. Herramientas como [ARMClient](https://github.com/projectkudu/ARMClient) y otras pueden controlar la autorización automáticamente y se recomiendan para principiantes.
...
El primer paso para definir un patrón estándar de cumplimiento es elaborar un plano técnico a partir de los recursos disponibles. En este ejemplo, cree un plano técnico llamado MyBlueprint para configurar las asignaciones de roles y directivas para la suscripción, agregue un grupo de recursos y cree una plantilla de Resource Manager y una asignación de roles en el grupo de recursos.
...
1. Cree el objeto _blueprint_ inicial. El **cuerpo de la solicitud** incluye propiedades sobre el plano técnico, cualquier grupo de recursos que se vaya a crear y todos los parámetros de nivel del plano técnico que se establecen durante la asignación y que usan los artefactos agregados en pasos posteriores.
...
1. Agregue la asignación de directivas a la suscripción. El **cuerpo de la solicitud** define el _tipo_ de artefacto, las propiedades que se alinean con una definición de directiva o iniciativa, y configura la asignación de la directiva para que utilice los parámetros definidos del plano técnico que se van a configurar durante la asignación del plano técnico.
...
1. Agregue otra asignación de directiva para la etiqueta Storage (reutilizando el parámetro _storageAccountType_) en la suscripción. Este artefacto de asignación de directiva adicional muestra que un parámetro definido en el plano técnico lo puede utilizar más de un artefacto. En el ejemplo, el parámetro **storageAccountType** se usa para establecer una etiqueta en el grupo de recursos que proporciona información sobre la cuenta de almacenamiento que se crea en el paso siguiente.
...
1. Agregue la plantilla en el grupo de recursos. El **cuerpo de la solicitud** de una plantilla de Resource Manager incluye el componente JSON normal de la plantilla, define el grupo de recursos de destino con **properties.resourceGroup** y reutiliza los parámetros de plano técnico **storageAccountType**, **tagName** y **tagValue** al proporcionar cada uno de ellos a la plantilla. Los parámetros del plano técnico se ponen a disposición de la plantilla mediante la definición de **properties.parameters** y dentro de la plantilla JSON el par clave/valor se utiliza para insertar el valor. Los nombres de los parámetros del plano técnico y de la plantilla podrían ser los mismos, pero se han hecho diferentes para ilustrar cómo se pasa cada uno de ellos del plano técnico al artefacto de la plantilla.
...
Los planos técnicos se pueden quitar de una suscripción si ya no se necesitan o se han reemplazado por planos técnicos más nuevos con patrones, directivas y diseños actualizados. Cuando se quita un plano técnico, se omiten los artefactos que se asignaron como parte de ese plano técnico. Para quitar una asignación del plano técnico, use la siguiente operación de la API REST:
