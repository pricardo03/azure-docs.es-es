---
title: Serialización de caché de tokens en Microsoft Authentication Library for .NET | Azure
...
Las estrategias son diferentes en función de si se escribe una serialización de la caché de tokens para un [aplicación cliente pública](msal-client-applications.md) (escritorio) o un [aplicación cliente confidencial](msal-client-applications.md)) (aplicación web/API web o aplicación de demonio).
...
Algo que es importante recordar es que para las aplicaciones web y las API web, debe haber una caché de tokens por usuario (por cuenta). Es preciso serializar la caché de tokens de cada cuenta.
...
| ------ | -------- | ----------- |
|[active-directory-dotnet-desktop-msgraph-v2](http://github.com/azure-samples/active-directory-dotnet-desktop-msgraph-v2) | Escritorio (WPF) | Aplicación .NET de Windows Desktop (WPF) que llama a la API Microsoft Graph. ![Topología](media/msal-net-token-cache-serialization/topology.png)|
