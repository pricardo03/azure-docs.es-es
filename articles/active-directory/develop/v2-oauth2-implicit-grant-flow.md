...
![Calles OpenId Connect](./media/v2-oauth2-implicit-grant-flow/convergence_scenarios_implicit.png)
...
| ID_token |El id_token que solicitó la aplicación. Puede usar el id_token para comprobar la identidad del usuario y comenzar una sesión con el usuario. En [Protocolos de la versión 2.0: OpenID Connect](v2-id-and-access-tokens.md), se incluyen más detalles acerca de id_tokens y su contenido. |
...
Puede elegir validar el `id_token` en el código de cliente, pero lo habitual es enviar el `id_token` a un servidor back-end y realizar allí la validación. Una vez haya validado la firma del id_token, se le solicitará que compruebe algunas notificaciones: Para más información, consulte [Referencia de los tokens de v2.0](v2-id-and-access-tokens.md), incluidas las secciones [Validación de los tokens](v2-id-and-access-tokens.md#validating-tokens) e [Información importante sobre la sustitución de la clave de firma](v2-id-and-access-tokens.md#validating-tokens). Hay al menos una disponible para la mayoría de los lenguajes y las plataformas.
...
Para más información sobre las notificaciones en un id_token, consulte [Referencia de los tokens de v2.0](v2-id-and-access-tokens.md).
...
| redirect_uri |recomendado |El redirect_uri de su aplicación, a donde su aplicación puede enviar y recibir las respuestas de autenticación. Debe coincidir exactamente con uno de los redirect_uris que registró en el portal, con la excepción de que debe estar codificado como URL. |
| ámbito |requerido |Una lista de ámbitos separada por espacios. Para obtener los tokens, incluya todos los [ámbitos](v2-permissions-and-consent.md) que necesita para el recurso de interés. |
...
Para más información sobre las notificaciones presentes en el token de acceso, consulte la [referencia del token de punto de conexión v2.0](v2-id-and-access-tokens.md).
