...
description: Uso de los id_tokens emitidos por los puntos de conexión de las versiones v1 y v2 de Azure AD.
...
## <a name="validating-idtokens"></a>Validación de `id_tokens`

La validación de un id_token es muy similar al primer paso de la [validación de un token de acceso](access-tokens.md#validating-tokens): el cliente debe validar que el emisor correcto ha enviado de vuelta el token y que no se ha manipulado.  Dado que los `id_tokens` siempre son un token JWT, existen muchas bibliotecas para validar estos tokens: se recomienda usar una de ellas en lugar de hacerlo usted mismo.  
