...
| `availableToOtherTenants` | Boolean | True si la aplicación se comparte con otros inquilinos; en caso contrario, false. <br><br> _Nota: Esto se sustituye por `signInAudience` en el [registros de aplicaciones (versión preliminar)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) experimentar._ | |
...
| `displayName` | string | El nombre para mostrar de la aplicación. <br><br> _Nota: Esto se sustituye por `name` en el [registros de aplicaciones (versión preliminar)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) experimentar._ | `"MyRegisteredApp"` |
...
| `homepage` | string | URL a la página principal de la aplicación. <br><br> _Nota: Esto se sustituye por `signInUrl` en el [registros de aplicaciones (versión preliminar)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) experimentar._ | `"https://MyRegisteredApp"` |
| `objectId` | string | El identificador único de la aplicación en el directorio. <br><br> _Nota: Esto se sustituye por `id` en el [registros de aplicaciones (versión preliminar)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) experimentar._ | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `publicClient` | Boolean | Especifica si esta aplicación es un cliente público (por ejemplo, una aplicación instalada que se ejecutan en un dispositivo móvil). <br><br> _Nota: Esto se sustituye por `allowPublicClient` en el [registros de aplicaciones (versión preliminar)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) experimentar._ | |
| `publisherDomain` | string | El dominio del publicador comprobado para la aplicación. Es de sólo lectura. | https://www.contoso.com |
| `replyUrls` | Matriz de cadena | Esta propiedad multivalor contiene la lista de valores de redirect_uri registrados que Azure AD aceptará como destinos cuando se devuelvan tokens. <br><br> _Nota: Esto se sustituye por `replyUrlsWithType` en el [registros de aplicaciones (versión preliminar)](https://developer.microsoft.com/en-us/graph/blogs/new-app-registration/) experimentar._ | |
...
1. Si el flujo de trabajo, deberá guardar los manifiestos en el repositorio de código fuente para su uso posterior, se recomienda reorganizando los manifiestos de guardado en el repositorio con el que ve en el **registros de aplicaciones (versión preliminar)** experimentar.
