...
Si tiene una VM que ya no necesita la identidad administrada asignada por el sistema:
...
Si tiene una VM que ya no necesita la identidad administrada asignada por el usuario:

1. Independientemente de que inicie sesión localmente en Azure o mediante Azure Portal, use una cuenta que esté asociada a la suscripción de Azure que contiene la máquina virtual.

2. Cargue la plantilla en un [editor](#azure-resource-manager-templates) y busque el `Microsoft.Compute/virtualMachines`recurso de interés dentro de la sección `resources`. Si dispone de una VM que solo tenga una identidad administrada asignada por el usuario, puede deshabilitarla cambiando el tipo de identidad a `None`.
   ...
   **Microsoft.Compute/virtualMachines versión 2017-12-01 de la API**
   ...
   Para quitar una identidad administrada asignada por un usuario único de una VM, elimínela de la matriz `identityIds`.
