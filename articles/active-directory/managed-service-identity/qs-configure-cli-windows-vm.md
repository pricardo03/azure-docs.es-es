...
- Si no está familiarizado con las identidades administradas para recursos de Azure, consulte la [sección de introducción](overview.md). **No olvide revisar la [diferencia entre una identidad administrada asignada por el sistema y una identidad administrada asignada por el usuario](overview.md#how-does-it-work)**.
...
Para quitar la extensión de VM de identidad administrada para recursos de Azure, el usuario `-n ManagedIdentityExtensionForWindows` o el modificador `-n ManagedIdentityExtensionForLinux` (en función del tipo de VM) con [az vm extension delete](https://docs.microsoft.com/cli/azure/vm/#assign-identity):
...
2. Cree una identidad administrada asignada por el usuario mediante [az identity create](/cli/azure/identity#az-identity-create).  El parámetro `-g` especifica el grupo de recursos donde se crea la identidad administrada asignada por el usuario, mientras que el parámetro `-n` especifica su nombre.    
