...
- Si ejecuta PowerShell localmente, también necesita realizar lo siguiente: 
    - Ejecute `Login-AzureRmAccount` para crear una conexión con Azure.
    - Instale la [versión más reciente de PowerShellGet](/powershell/gallery/installing-psget#for-systems-with-powershell-50-or-newer-you-can-install-the-latest-powershellget).
    - Ejecute `Install-Module -Name PowerShellGet -AllowPrerelease` para obtener la versión preliminar del módulo `PowerShellGet` (es posible que deba `Exit` fuera de la sesión actual de PowerShell después de ejecutar este comando para instalar el módulo `AzureRM.ManagedServiceIdentity`).
    - Ejecute `Install-Module -Name AzureRM.ManagedServiceIdentity -AllowPrerelease` para instalar la versión preliminar del módulo `AzureRM.ManagedServiceIdentity`, lo que le permitirá llevar a cabo las operaciones de identidad asignada por el usuario en este artículo.
