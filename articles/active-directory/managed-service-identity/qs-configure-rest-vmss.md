...
3. Para quitar la identidad asignada por el sistema de un conjunto de escalado de máquinas virtuales que tiene identidades asignadas por el usuario, quite `SystemAssigned` del valor `{"identity":{"type:" "}}` mientras mantiene el valor `UserAssigned` y la matriz `identityIds` que define qué identidades asignadas por el usuario se asignan al conjunto de escalado de máquinas virtuales.
...
5. Si tiene asignadas identidades asignadas por el usuario o por el sistema al conjunto de escalado de máquinas virtuales, debe agregar la nueva identidad asignada por el usuario a la matriz `identityIDs`, mientras conserva al mismo tiempo las identidades asignadas por el usuario y por el sistema que están actualmente asignadas al conjunto de escalado de máquinas virtuales.

   Por ejemplo, si tiene una identidad asignada por el sistema y una identidad asignada por el usuario `ID1` actualmente asignadas al conjunto de escalado de máquinas virtuales y desea agregarles la identidad de usuario `ID2`, utilice el siguiente comando CURL. Reemplace `<ACCESS TOKEN>` por el valor que ha recibido en los pasos cuando solicitó un token de acceso de portador y el valor `<SUBSCRIPTION ID>` según sea apropiado para su entorno.
...
2. Para asegurarse de que no elimina ninguna identidad administrada asignada por el usuario o por el sistema que está asignada al conjunto de escalado de máquinas virtuales y de que no quita la identidad asignada por el sistema, necesita enumerar las identidades administradas mediante el comando CURL siguiente: 
   ...
   Por ejemplo, si tiene asignadas identidades asignadas por el usuario `ID1` y `ID2` al conjunto de escalado de máquinas virtuales, y solo desea mantener `ID1` asignada y conservar la identidad asignada por el sistema, debe utilizar el mismo comando CURL que si asignara una identidad administrada asignada por el usuario a un conjunto de escalado de máquinas virtuales solo manteniendo el valor `ID1` y el valor `SystemAssigned`. Esto quita la identidad asignada por el usuario `ID2` del conjunto de escalado de máquinas virtuales, mientras conserva al mismo tiempo la identidad asignada por el sistema.
