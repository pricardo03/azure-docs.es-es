...
Consulte [conectores comprender el Proxy de aplicación de Azure AD](application-proxy-connectors.md) para una descripción más detallada.
   ...
   * Conector hospeda debe [habilitarse para TLS 1.2](application-proxy-add-on-premises-application.md) antes de instalar los conectores.

   * Si es posible, implementar conectores en el [misma red](application-proxy-network-topology.md) y segmento, como los servidores de aplicaciones web de back-end. Es mejor implementar hosts de conector después de completar una detección de aplicaciones.

* **Configuración de acceso de red**: Los conectores de Azure AD Application Proxy [intentan conectarse a Azure a través de HTTPS (puerto TCP 443) y HTTP (puerto TCP 80)](application-proxy-add-on-premises-application.md). 
   ...
   * Equilibrio de carga de los conectores del Proxy a sí mismos también no es compatible, o incluso necesarios.
...
* **Roles y derechos administrativos**
   ...
   * **Instalación del conector** requiere derechos de administrador local en el servidor de Windows que se está instalando en. También requiere un mínimo de un rol de administrador de la aplicación para autenticarse y registrar la instancia del conector para su inquilino de Azure AD. 
...
* Es posible que deban una elevación de rol para obtener derechos de administrador de la aplicación a través de [Privileged Identity Manager](https://docs.microsoft.com/azure/active-directory/privileged-identity-management/pim-configure) (PIM), por lo que asegúrese de que su cuenta es apta. 
...
* Dominio y usuarios de Azure AD pueden tener acceso a las aplicaciones publicadas de forma segura con conexión directa single sign-on (SSO) cuando utilizando Azure AD ni Unidos a un dominio unido dispositivos.

* Los usuarios con dispositivos personales autorizados pueden acceso seguro a aplicaciones publicadas siempre que están inscritos en MFA y han registrado la aplicación Microsoft Authenticator en su teléfono móvil como método de autenticación.
...
* **Usar grupos de conectores**: Asignar a un grupo de conectores que se ha designado para la publicación de cada aplicación.
...
Para escenarios donde las aplicaciones publican de vínculos a otra aplicación publicada, habilitar la traducción de vínculos o cada aplicación para que tenga control sobre la experiencia del usuario en el nivel por aplicación.
...
Compruebe que la aplicación es accesible a través del Proxy de aplicación. 
...
Con la autenticación previa habilitada, Azure AD desafiará para la autenticación y, a continuación, la aplicación back-end debe también desafío si requiere autenticación. También se cambia la autenticación previa de acceso directo a Azure AD configura la dirección URL externa con HTTPS, por lo que cualquier aplicación que se configura inicialmente para HTTP ahora estará protegido con HTTPS.
...
Realizar el inicio de sesión único solo es posible que si Azure AD pueda identificar el usuario solicita acceso a un recurso, por lo que la aplicación debe configurarse para autenticar previamente los usuarios al acceso de inicio de sesión único funcione, en caso contrario, se deshabilitarán las opciones de inicio de sesión único.
...
Estos registros detallan los inicios de sesión para las aplicaciones configuradas con el Proxy de aplicación, así como información sobre el dispositivo y el usuario tiene acceso a la aplicación. Se encuentran en el portal de Azure y en la API de auditoría.
...
Estos artículos cubren escenarios comunes, pero también puede crear a sus propias guías de solución de problemas para su organización de soporte técnico. 
