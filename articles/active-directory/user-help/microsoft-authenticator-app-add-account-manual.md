...
ms.locfileid: "55478107"
---
# <a name="manually-add-an-account-to-the-app"></a>Agregar manualmente una cuenta a la aplicación
Si la cámara no puede capturar el código QR, puede agregar manualmente la información de su cuenta a la aplicación Microsoft Authenticator para realizar la verificación en dos fases. Esta opción funciona para las cuentas profesionales o educativas y las cuentas personales que no sean de Microsoft.
...
1. En su PC, fíjese en el **código** y la **dirección URL** que se encuentra en la página para **configurar la aplicación móvil**. Mantenga esta página abierta para que pueda ver el código y la dirección URL.
    ...
    ![Pantalla que proporciona el código QR](./media/microsoft-authenticator-app-add-account-manual/auth-app-barcode.png)
...
3. Seleccione **OR ENTER CODE MANUALLY** (O BIEN ESCRIBA EL CÓDIGO MANUALMENTE).
    ...
    ![Pantalla para digitalizar un código QR](./media/microsoft-authenticator-app-add-account-manual/auth-app-manual-code.png)
   
4. Escriba el **código** y la **dirección URL** del paso 1 y, a continuación, seleccione **Finish** (Finalizar).

    ![Pantalla para escribir el código y la dirección URL](./media/microsoft-authenticator-app-add-account-manual/auth-app-code-url.png)
...
5. En la página **Add a backup verification method** (Agregar un método de verificación de copia de seguridad) en su PC, escriba el código de verificación de seis dígitos que proporciona el método de verificación de copia de seguridad para su cuenta de Amazon y, a continuación, seleccione **Verify code and continue** (Comprobar el código y continuar).
...
6. En la página Almost done (Casi hemos terminado), decida si quiere que su PC sea un dispositivo de confianza y, a continuación, seleccione **Got it. Turn on Two-Step Verification** (Entendido. Activar la verificación en dos pasos).
    ...
    A continuación aparecerá la página **Advanced Security Settings** (Configuración de seguridad avanzada), y se mostrarán los detalles de la verificación en dos fases actualizada.
...
- Después de agregar las cuentas a la aplicación, puede iniciar sesión con la aplicación Microsoft Authenticator en su dispositivo. Para obtener más información, consulte cómo [iniciar sesión con la aplicación](microsoft-authenticator-app-phone-signin-faq.md).
