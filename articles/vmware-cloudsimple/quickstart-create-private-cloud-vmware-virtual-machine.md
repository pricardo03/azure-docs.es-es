---
title: 'Inicio rápido: Creación de una máquina virtual de VMware en una nube privada'
...
> [!WARNING]
> Para las cargas ISO, use el cliente HTML5 de vSphere.  El uso del cliente Flash puede producir un error.

1. Obtenga la plantilla de vSphere o ISO que quiera cargar a vCenter para crear una máquina virtual y póngala a disposición del sistema local.
...
2. En vCenter, haga clic en el icono **Disk** (Disco) y seleccione **vsanDatastore**. Haga clic en **Files** (Archivos) y después en **New Folder** (nueva carpeta).
    ...
    ![ISO de vCenter](media/vcenter-create-folder.png)

3. Cree una carpeta para almacenar los archivos ISO.

4. Vaya a la nueva carpeta creada y haga clic en **Cargar archivos**. Siga las instrucciones en pantalla para cargar el archivo ISO.
...
3. Seleccione **Create new virtual machine** (Crear nueva máquina virtual) y haga clic en **Next** (Siguiente).
    ...
    ![Asistente para crear nueva máquina virtual](media/create-vcenter-virtual-machine-02.png)

4. Asigne un nombre a la máquina virtual, seleccione la carpeta de **la máquina virtual de la carga de trabajo** y haga clic en **Next** (Siguiente).

    ![Selección del nombre y la carpeta](media/create-vcenter-virtual-machine-03.png)

5. Seleccione el recurso de proceso **Workload** (Carga de trabajo) y haga clic en **Next** (Siguiente).

    ![Selección del recurso de proceso](media/create-vcenter-virtual-machine-04.png)
...
7. Mantenga la selección de compatibilidad de ESXi 6.5 predeterminada y haga clic en **Next** (Siguiente).
    ...
    ![Selección de la compatibilidad](media/create-vcenter-virtual-machine-06.png)

8. Seleccione el sistema operativo invitado del archivo ISO para la máquina virtual y haga clic en **Next** (Siguiente).

    ![Personalización del sistema operativo invitado](media/create-vcenter-virtual-machine-07.png)

9. Seleccione las opciones de disco duro y de red. Para la nueva unidad de CD/DVD, seleccione **Datastore ISO file** (Archivo ISO de almacén de datos).  Si quiere permitir el tráfico desde la dirección IP pública a esta máquina virtual, seleccione la red como **vm-1**.

    ![Selección de la personalización de hardware](media/create-vcenter-virtual-machine-08.png)
...
10. Se abrirá una ventana de selección. Seleccione el archivo que ha cargado previamente en la carpeta de archivos ISO y plantillas, y haga clic en **Aceptar**.
    ...
    ![Selección de ISO](media/create-vcenter-virtual-machine-10.png)

11. Revise la configuración y haga clic en **Aceptar** para crear la máquina virtual.

    ![Revisión de las opciones](media/create-vcenter-virtual-machine-11.png)

La máquina virtual ya se ha agregado a los recursos de proceso de carga de trabajo y está lista para su uso. 

![Máquina virtual nueva en vCenter](media/create-vcenter-virtual-machine-12.png)

La configuración básica ya se ha completado. Puede empezar a usar la nube privada propia de la misma forma que usaría la infraestructura de máquina virtual local.
...
En estos artículos de Microsoft se describe cómo configurar un servidor de Windows como un servidor DNS y como un servidor DHCP.
<br>
...
Las páginas de red del portal de CloudSimple permiten especificar la configuración para las tablas de firewall y las direcciones IP públicas para las máquinas virtuales.
...
4. Seleccione la ubicación de la nube privada.
...
La máquina virtual a la que se debe asignar esta dirección IP se tiene que configurar con la dirección local especificada antes. El procedimiento para configurar una dirección IP es específico del sistema operativo de la máquina virtual. Consulte la documentación del sistema operativo de la máquina virtual para obtener el procedimiento correcto.
...
Actualice la sección siguiente en el archivo ```interfaces```.
...
De forma predeterminada, se **deniega** todo el tráfico entrante desde Internet. Si quiere abrir cualquier otro puerto, cree una [tabla de firewall](https://docs.azure.cloudsimple.com/firewall/).
...
Asegúrese de que las reglas de firewall (iptable) en la máquina virtual no bloquean el puerto 80 entrante.
...
Inicie un explorador en el escritorio y apúntelo al puerto 80 para que la dirección IP pública examine los archivos en la máquina virtual. 
...
También puede modificar la forma en que se protege el tráfico, mediante la característica Reglas de firewall. Para más información, vea [Configuración de tablas y reglas de firewall](https://docs.azure.cloudsimple.com/firewall/).
...
Para instalar una solución, debe solicitar privilegios adicionales durante un período limitado. Vea [Escalado de privilegios](https://docs.azure.cloudsimple.com/escalate-private-cloud-privileges).
...
* [Configuración de puertas de enlace de VPN en la red de CloudSimple](https://docs.azure.cloudsimple.com/vpn-gateway)
