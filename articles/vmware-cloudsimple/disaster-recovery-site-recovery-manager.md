---
title: 'Azure VMware Solutions (AVS): configuración de una nube privada de AVS como sitio de recuperación ante desastres mediante VMware Site Recovery Manager'
...
Puede usar la nube privada de AVS como un sitio de recuperación ante desastres (DR) para las cargas de trabajo locales de VMware.

La solución de recuperación ante desastres se basa en vSphere Replication y VMware Site Recovery Manager (SRM). Se puede seguir un enfoque similar para habilitar la nube privada de AVS como sitio primario protegido por el sitio de recuperación local.
...
* Instale, configure y administre vSphere Replication y SRM en su nube privada de AVS.
* Proporcione sus propias licencias para SRM cuando la nube privada de AVS sea el sitio protegido. No necesita ninguna licencia de SRM adicional para el sitio de AVS cuando se usa como sitio de recuperación.
...
Puede usar cualquier versión de vRA y SRM que sea compatible con la nube privada de AVS y los entornos locales. En los ejemplos de esta guía se usan vRA 6.5 y SRM 6.5. Estas versiones son compatibles con vSphere 6.5, que también es compatible con AVS.

## <a name="deploy-the-solution"></a>Implementación de la solución

En las secciones siguientes se describe cómo implementar una solución de recuperación ante desastres con SRM en la nube privada de AVS.
...
4. [Configurar redes de la nube privada de AVS para la solución de SRM](#set-up-avs-private-cloud-networking-for-the-srm-solution)
5. [Configurar una conexión VPN de sitio a sitio entre la red local y la nube privada de AVS y abrir los puertos necesarios](#set-up-a-site-to-site-vpn-connection-between-your-on-premises-network-and-the-avs-private-cloud-and-open-required-ports)
6. [Configurar los servicios de infraestructura en la nube privada de AVS](#set-up-infrastructure-services-in-your-avs-private-cloud)
...
8. [Instalar vSphere Replication Appliance en el entorno de la nube privada de AVS](#install-vsphere-replication-appliance-in-your-avs-private-cloud-environment)
9. [Instalar el servidor de SRM en el entorno local](#install-srm-server-in-your-on-premises-environment)
10. [Instalar el servidor de SRM en la nube privada de AVS](#install-srm-server-in-your-avs-private-cloud)
...
* Debe implementarse la misma versión de SRM en la nube privada de AVS y en el entorno local.
* Debe implementarse la misma versión de vSphere Replication en la nube privada de AVS y en el entorno local.
* Las versiones de Platform Services Controller (PSC) de la nube privada de AVS y el entorno local deben ser compatibles.
* Las versiones de vCenter de la nube privada de AVS y el entorno local deben ser compatibles.
...
Para averiguar las versiones de vCenter y PSC de su nube privada de AVS, abra el portal de AVS. Vaya a **Recursos**, seleccione la nube privada de AVS y haga clic en la pestaña **Red de administración de vSphere**.

![Versiones de vCenter y PSC en la nube privada de AVS](media/srm-resources.png)
...
3. Use la herramienta calibradora de AVS para calcular los recursos que se necesitan en el sitio de recuperación ante desastres para proteger su entorno local.

### <a name="create-an-avs-private-cloud-for-your-environment"></a>Crear una nube privada de AVS para su entorno

Cree una nube privada de AVS desde el portal de AVS siguiendo las instrucciones y las recomendaciones de ajuste de tamaño que se proporcionan en el artículo [Crear una nube privada de AVS](create-private-cloud.md).

### <a name="set-up-avs-private-cloud-networking-for-the-srm-solution"></a>Configurar redes de la nube privada de AVS para la solución de SRM

Acceda al portal de AVS para configurar las redes de la nube privada de AVS para la solución de SRM.
...
### <a name="set-up-a-site-to-site-vpn-connection-between-your-on-premises-network-and-the-avs-private-cloud-and-open-required-ports"></a>Configurar una conexión VPN de sitio a sitio entre la red local y la nube privada de AVS y abrir los puertos necesarios

Puede configurar la conectividad de sitio a sitio entre la red local y la nube privada de AVS. Para obtener instrucciones, consulte [Configuración de una conexión VPN en la nube privada de AVS](set-up-vpn.md).

### <a name="set-up-infrastructure-services-in-your-avs-private-cloud"></a>Configurar los servicios de infraestructura en la nube privada de AVS
...
Puede agregar un proveedor de identidades externo tal y como se describe en el artículo [Uso de Azure AD como proveedor de identidades para vCenter en la nube privada de AVS](azure-ad.md) si quiere realizar alguna de las acciones siguientes:

* Identificar a los usuarios de su instancia local de Active Directory (AD) en la nube privada de AVS.
* Configurar una instancia de AD en la nube privada de AVS para todos los usuarios.
* Usar Azure AD.
...
## <a name="install-vsphere-replication-appliance-in-your-avs-private-cloud-environment"></a>Instalar vSphere Replication Appliance en el entorno de la nube privada de AVS

Antes de comenzar, verifique que dispone de lo siguiente:

* Disponibilidad de IP desde las subredes del entorno local a la subred de administración de la nube privada de AVS
* Disponibilidad de IP desde la subred de replicación del entorno local de vSphere a la subred de la solución de SRM de la nube privada de AVS

Para obtener instrucciones, consulte [Configuración de una conexión VPN en la nube privada de AVS](set-up-vpn.md). Los pasos son similares a los de la instalación local.

AVS recomienda el uso de FQDN en lugar de direcciones IP durante la instalación de vRA y SRM. Para averiguar el FQDN de vCenter y PSC de su nube privada de AVS, abra el portal de AVS. Vaya a **Recursos**, seleccione la nube privada de AVS y haga clic en la pestaña **Red de administración de vSphere**.

![Búsqueda del FQDN de vCenter y PSC en la nube privada de AVS](media/srm-resources.png)

AVS requiere que no instale vRA y SRM con el usuario "cloudowner" predeterminado, sino que, en su lugar, cree un nuevo usuario. Esto permite garantizar un tiempo de actividad y una disponibilidad elevados para el entorno de vCenter de la nube privada de AVS. Sin embargo, el usuario cloudowner predeterminado de la instancia de vCenter de la nube privada de AVS no tiene privilegios suficientes para crear un nuevo usuario con privilegios administrativos.

Antes de instalar vRA y SRM, debe escalar los privilegios de vCenter del usuario cloudowner y, a continuación, crear un usuario con privilegios administrativos en el dominio de SSO de vCenter. Para obtener detalles sobre el modelo predeterminado de permisos y usuarios de la nube privada de AVS, consulte [Información sobre el modelo de permisos de la nube privada de AVS](learn-private-cloud-permissions.md).
...
2. Cree un usuario en la nube privada de AVS para la instalación de SRM y vSphere Replication. Se explica más abajo en la sección [Interfaz de usuario de vCenter: crear un usuario en la nube privada de AVS para la instalación de vRA y SRM](#vcenter-ui-create-a-user-in-the-avs-private-cloud-for-vra-and-srm-installation).
3. Prepare el entorno de la nube privada de AVS para la instalación de vRA.
4. Implemente vRA en su nube privada de AVS mediante el uso de OVF en el archivo ISO de VR desde vmware.com. En el caso de vRA 6.5, puede encontrar información pertinente en [este blog de VMware](https://blogs.vmware.com/virtualblocks/2017/01/20/vr-65-ovf-choices).
5. Configure las reglas del firewall para vRA. Se explica más abajo en [Portal de AVS: configurar las reglas de firewall para vRA](#avs-portal-configure-firewall-rules-for-vra).
6. Registre la instancia de vRA de la nube privada de AVS con el inicio de sesión único de vCenter en el sitio de la nube privada de AVS.
...
#### <a name="vcenter-ui-create-a-user-in-the-avs-private-cloud-for-vra-and-srm-installation"></a>Interfaz de usuario de vCenter: crear un usuario en la nube privada de AVS para la instalación de vRA y SRM

Inicie sesión en vCenter con las credenciales del usuario cloudowner después de escalar los privilegios desde el portal de AVS.
...
#### <a name="avs-portal-configure-firewall-rules-for-vra"></a>Portal de AVS: Configuración de las reglas de firewall para vRA
...
* vSphere Replication Appliance está instalado en los entornos de nube privada de AVS y local.
...
### <a name="install-srm-server-in-your-avs-private-cloud"></a>Instalar el servidor de SRM en la nube privada de AVS
...
En los pasos siguientes se describe la instalación de SRM de la nube privada de AVS.

1. [Interfaz de usuario de vCenter: instalar SSMS](#vcenter-ui-install-srm)
2. [Portal de AVS: configurar las reglas de firewall para SRM](#avs-portal-configure-firewall-rules-for-srm)
3. [Interfaz de usuario de vCenter: configurar SRM](#vcenter-ui-configure-srm)
4. [Portal de AVS: anular el escalado de privilegios](#avs-portal-de-escalate-privileges)
...
#### <a name="avs-portal-configure-firewall-rules-for-srm"></a>Portal de AVS: configurar las reglas de firewall para SRM
...
El servidor de SRM y vCenter o PSC en la nube privada de AVS.
...
* [Cálculo de los requisitos de ancho de banda para vSphere Replication](https://kb.vmware.com/s/article/2037268)
