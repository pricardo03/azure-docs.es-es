...
> [!NOTE]
> Cuando cree la entidad de servicio, debe ser el propietario o administrador de acceso de los usuarios de la suscripción que desee usar para la incorporación. Si no tiene permisos suficientes para crear asignaciones de roles, es posible que se cree la entidad de servicio, pero no podrá incorporar máquinas.
...
> [!NOTE] 
> Los permisos de SPN pueden tardar un tiempo en rellenarse correctamente. Ejecución de la siguiente asignación de roles para establecer los permisos mucho más rápidamente.
> ``` PowerShell
> New-AzRoleAssignment -RoleDefinitionName "Azure Connected Machine Onboarding" -ServicePrincipalName $sp.ApplicationId
> ```

Ahora, recupere la contraseña mediante PowerShell.
...
* La propiedad **ApplicationId** se utiliza para el parámetro `--service-principal-id` que se usa para conectar el agente.
* La propiedad **password** se utiliza para el parámetro `--service-principal-secret` que se usa para conectar el agente.

> [!NOTE]
> Asegúrese de usar la propiedad **ApplicationId** de la entidad de servicio, no la propiedad **Id**. **Id** no funcionará.
...
* `location`: consulte las [regiones y ubicaciones de Azure](https://azure.microsoft.com/global-infrastructure/regions/). Esta ubicación puede ser la misma, u otra diferente, que la del grupo de recursos. En la versión preliminar pública, el servicio se admite en **Oeste de EE. UU. 2**, **Sudeste Asiático** y **Oeste de Europa**.
...
* Si ya ha instalado el agente, ejecute el comando `/opt/azcmagent/bin/hybridrp_proxy add http://{proxy-url}:{proxy-port}`, que configura el proxy y reinicia el agente.
