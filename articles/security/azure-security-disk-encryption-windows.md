...
| keyVaultURL | Dirección URL del almacén de claves en el que se que debe cargar la clave de BitLocker. Puede obtenerla mediante el cmdlet `(Get-AzureRmKeyVault -VaultName,-ResourceGroupName).VaultURI` o la CLI de Azure `az keyvault show --name "MySecureVault" --query properties.vaultUri` |
...
### <a name="bkmk_VHDpreRM"> </a>Cifrado de máquinas virtuales IaaS con disco duros virtuales previamente cifrados con una plantilla de Resource Manager 
...
Puede usar la autenticación de certificado de cliente con o sin KEK. Para usar los scripts, es necesario cumplir los [requisitos previos de Azure Disk Encryption](azure-security-disk-encryption-prerequisites.md). Antes de usar los scripts de PowerShell, ya debe tener el certificado cargado en el almacén de claves e implementado en la máquina virtual. Si también usa KEK, ya debe existir KEK. Para más información, consulte la sección [Autenticación basada en certificados de Azure AD](azure-security-disk-encryption-prerequisites.md#bkmk_Cert) del artículo de requisitos previos.
