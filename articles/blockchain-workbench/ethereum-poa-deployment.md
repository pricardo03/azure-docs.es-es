---
title: Consorcio de prueba de autoridad de Ethereum
...
Dado que cada miembro tendrá varios nodos de validador ejecutándose simultáneamente y cada nodo debe tener una identidad con permisos, es importante que los validadores puedan adquirir de forma segura una identidad activa única en la red. Para facilitar esto, hemos creado un almacén de identidades que se implementa en la suscripción de cada miembro que contiene de forma segura las identidades de Ethereum generadas. Tras la implementación, el contenedor de orquestación generará una clave privada de Ethereum para cada validador y la almacenará en Azure Key Vault. Antes de que el nodo de paridad se inicie, primero adquiere una concesión en una identidad no utilizada para asegurar que la identidad no sea captada por otro nodo. La identidad se proporciona al cliente, lo que la da la autoridad para comenzar a crear bloques. Si la máquina virtual anfitriona experimenta una interrupción, la concesión se liberará, permitiendo que un nodo de reemplazo reanude su identidad en el futuro.

### <a name="bootnode-registrar"></a>Registrador de nodos de arranque

Para habilitar la facilidad de conectividad, cada miembro hospedará un conjunto de información de conexión en el [punto de conexión de API de datos](#data-api). Estos datos incluyen una lista de nodos de arranque que se proporcionan como nodos del mismo nivel para el miembro de combinación. Como parte de esta API de datos, mantenemos esta lista de nodos de arranque actualizada

### <a name="bring-your-own-operator"></a>Traiga su propio operador

A menudo, un miembro del consorcio querrá participar en el gobierno de la red, pero no quiere operar ni mantener su infraestructura. A diferencia de los sistemas tradicionales, tener un único operador en la red funciona en contra del modelo descentralizado de sistemas de cadena de bloques. En lugar de alquilar un intermediario centralizado para operar una red, cada miembro del consorcio puede delegar la administración de la infraestructura al operador que prefiera. Esto permite un modelo híbrido donde cada miembro puede optar por operar su propia infraestructura o delegar la operación a un socio diferente. El flujo de trabajo de la operación delegada funciona de la siguiente manera:
...
![Traiga su propio operador](./media/ethereum-poa-deployment-guide/bring-you-own-operator.png)

### <a name="azure-monitor"></a>Azure Monitor

Esta solución también viene con Azure Monitor para realizar el seguimiento de las estadísticas de nodo y de red. Para los desarrolladores de aplicaciones, esto proporciona visibilidad en la cadena de bloques subyacente para realizar el seguimiento de las estadísticas de generación de bloques. Los operadores de red pueden usar Azure Monitor para detectar y evitar rápidamente las interrupciones de la red a través de las estadísticas de infraestructura y los registros consultables. Consulte [Supervisión de servicios](#service-monitoring) para obtener más detalles.
...
-   Conjuntos de escalado de máquina virtual (VMSS) para ejecutar los validadores de prueba de autoridad
...
![arquitectura de implementación](./media/ethereum-poa-deployment-guide/deployment-architecture.png)
...
En el corazón de la prueba de autoridad está el gobierno descentralizado. La aplicación descentralizada de gobierno es un conjunto de contratos inteligentes implementados previamente y una aplicación web que se usan para controlar las autoridades en la red.
...
![aplicación descentralizada de gobierno](./media/ethereum-poa-deployment-guide/governance-dapp.png)
...
Este proceso requiere una suscripción de Azure que puede admitir la implementación de varios conjuntos de escalado de máquinas virtuales y discos administrados. Si es necesario, [cree una cuenta de Azure](https://azure.microsoft.com/free/) gratuita para comenzar.
...
A continuación se muestra una implementación de ejemplo: ![Aspectos básicos](./media/ethereum-poa-deployment-guide/basic-blade.png)
...
A continuación se muestra una implementación de ejemplo: ![Regiones de implementación](./media/ethereum-poa-deployment-guide/deployment-regions.png)
...
A continuación se proporciona una descripción detallada de cada parámetro:
  ...
  Validator node storage performance (Rendimiento del almacenamiento de los nodos de validador)|Tipo de disco administrado que realiza una copia de seguridad de cada uno de los nodos de validador implementado.|Estándar o Premium|Estándar
...
A continuación se muestra una implementación de ejemplo: ![Tamaño de red y rendimiento](./media/ethereum-poa-deployment-guide/network-size-and-performance.png)
...
A continuación se proporciona una descripción detallada de cada parámetro:
  ...
  Nombre de parámetro|DESCRIPCIÓN|Valores permitidos|Valores predeterminados
  ---|---|---|---
Consortium Member ID (Identificador de miembro del consorcio)|El identificador asociado con cada miembro que participa en la red del consorcio usada para configurar los espacios de direcciones IP para evitar la colisión. En el caso de una red privada, el identificador de miembro debe ser único en las diferentes organizaciones de la misma red.  Es necesario un identificador de miembro único incluso si la misma organización se implementa en varias regiones. Tome nota del valor de este parámetro, ya que necesitará compartirlo con otros miembros que se estén uniendo para garantizar que no hay colisión.|0-255|N/D
...
Transaction Permission Contract (Contrato de permiso de transacciones) (Opciones avanzadas = Habilitar)|Código de bytes para el contrato de permisos de la transacción. Restringe la implementación de contrato inteligente y la ejecución a una lista con permisos de cuentas de Ethereum.|Código de bytes de contrato|N/D

A continuación se muestra una implementación de ejemplo: ![Configuración de Ethereum](./media/ethereum-poa-deployment-guide/ethereum-settings.png)

#### <a name="azure-monitor"></a>Azure Monitor

La hoja Azure Monitor le permite configurar un recurso de Azure Monitor para la red. Azure Monitor recopilará y expondrá métricas y registros útiles de la red, lo que proporciona la capacidad de comprobar rápidamente el estado de la red o los problemas de depuración.

  Nombre de parámetro|DESCRIPCIÓN|Valores permitidos|Valores predeterminados
  ---|---|---|---
Supervisión|Opción para habilitar Azure Monitor|Habilitar o Deshabilitar|Habilitar
Connect to existing Log Analytics instance? (¿Conectarse a instancia de Log Analytics existente?)|Crear una nueva instancia de Log Analytics como parte de Azure Monitor o unirse a una instancia existente|Crear nuevo o Unirse a existente|Crear nuevo
Azure Monitor Location (Ubicación de Azure Monitor) (Conectarse a Azure Monitor existente = Crear nuevo)|Región en la que se implementarán el nuevo Azure Monitor|Todas las regiones de Azure Monitor|N/D
Existing Log Analytics Workspace Id (Identificador de área de trabajo de Log Analytics existente) (Conectar a Azure Monitor existente = Unirse a existente)|Identificador del área de trabajo de la instancia de Log Analytics existente||N/D
Existing Log Analytics Primary Key (Clave principal de Log Analytics existente) (Conectarse a Azure Monitor existente = Unirse a existente)|Clave principal que se usa para conectarse a la instancia de Azure Monitor existente||N/D


A continuación se muestra una implementación de ejemplo: ![Azure Monitor](./media/ethereum-poa-deployment-guide/azure-monitor.png)
...
Una vez que la implementación haya finalizado, podrá acceder a los parámetros necesarios a través del correo electrónico de confirmación o a través de Azure Portal. En estos parámetros encontrará lo siguiente:
...
-   Identificador del recurso de puerta de enlace de red virtual (opcional)
...
![correo electrónico de implementación](./media/ethereum-poa-deployment-guide/deployment-email.png)

##### <a name="portal"></a>Portal

Una vez que la implementación haya finalizado correctamente y todos los recursos se hayan aprovisionado, podrá ver los parámetros de salida en el grupo de recursos.
...
Mediante la implementación basada en direcciones IP públicas, este primer paso no da problemas. Si realiza la implementación detrás de una VPN, consulte la sección [Conexión de puertas de enlace de red virtual](#connecting-vnet-gateways) para realizar la conexión de red como parte del [paso 2](#step-2-new-admin-deployment).

#### <a name="step-1-add-the-new-admin"></a>Paso 1: Agregar el nuevo administrador

1.  Recopile la dirección de Ethereum pública del nuevo administrador.

2.  Navegue hasta la aplicación descentralizada de gobierno y cree un nuevo administrador con la dirección de Ethereum y un alias adecuado.

3.  Notifique a los demás miembros existentes de la nueva solicitud para que puedan votar para agregar este nuevo administrador.

4.  Cuando el voto alcance el 51 %, el miembro se agregará como un administrador.

![agregar administrador](./media/ethereum-poa-deployment-guide/adding-admin.png)

#### <a name="step-2-new-admin-deployment"></a>Paso 2: Implementación del nuevo administrador
...
#### <a name="step-3-delegate-validators"></a>Paso 3: Delegar validadores

El nuevo administrador tendrá que autorizar a los validadores para participar en su nombre. Esto no es posible automáticamente ya que solo el miembro controla sus claves.\*

_\*Los nodos del primer miembro de la red se pueden agregar automáticamente porque podemos precompilar sus nodos de validador en el bloque de génesis en la red._

El nuevo administrador debe realizar los pasos siguientes:

1.  Abrir la aplicación descentralizada de gobierno implementada como parte de *su* implementación

2.  Navegar a la pestaña Validadores

3.  Seleccionar cada uno de los validadores que se han implementado y hacer clic en **Agregar**

![agregar validadores](./media/ethereum-poa-deployment-guide/adding-validators.png)

#### <a name="connecting-vnet-gateways"></a>Conexión de puertas de enlace de red virtual

En el caso de una red privada, los diferentes miembros se conectan a través de conexiones de puerta de enlace de red virtual. Antes de que un miembro pueda unirse a la red y ver el tráfico de transacción, un miembro existente debe realizar una configuración final en su puerta de enlace de VPN para aceptar la conexión. Esto significa que los nodos de Ethereum del miembro que se está uniendo no se ejecutarán hasta que se establezca una conexión. Se recomienda crear conexiones de red redundantes (malla) en el consorcio para reducir las posibilidades de un único punto de error.

Después de que el nuevo miembro se implemente, el miembro existente debe completar la conexión bidireccional configurando una conexión de puerta de enlace de red virtual con el nuevo miembro. Para lograr esto, el miembro existente necesitará:
...
El miembro existente debe ejecutar el siguiente script de PowerShell para completar la conexión. Se recomienda usar Azure Cloud Shell ubicado en la barra de navegación derecha superior en el portal.
...
![categorías de monitor](./media/ethereum-poa-deployment-guide/monitor-categories.png)
...
![estadísticas del nodo](./media/ethereum-poa-deployment-guide/node-stats.png)

Si selecciona **Network Stats** (Estadísticas de red) será útil para ver las estadísticas de red de Ethereum.

![estadísticas de red](./media/ethereum-poa-deployment-guide/network-stats.png)

#### <a name="sample-log-analytics-queries"></a>Consultas de Log Analytics de ejemplo
...
Por motivos de seguridad y de forma predeterminada, una regla de seguridad de grupo de red deniega el acceso al puerto SSH. Para acceder a instancias de conjunto de escalado de máquinas virtuales en la red de aplicación descentralizada, deberá cambiar esta regla a \"Permitir\"

1.  Empiece en la sección Información general del grupo de recursos implementados de Azure Portal.

    ![información general de ssh](./media/ethereum-poa-deployment-guide/ssh-overview.png)

2.  Seleccione el grupo de seguridad de red.

    ![nsg de ssh](./media/ethereum-poa-deployment-guide/ssh-nsg.png)
...
4.  Cambie \"Acción\" a Permitir.
    ...
    ![habilitar permitir en ssh](./media/ethereum-poa-deployment-guide/ssh-enable-allow.png)
...
El comando SSH que se debe ejecutar para acceder al primer nodo de validador se muestra en el parámetro de salida de la implementación de la plantilla como “SSH\_TO\_FIRST\_VL\_NODE\_REGION1” (para la implementación de ejemplo: ssh - p 4000 poaadmin\@leader4vb.eastus.cloudapp.azure.com). Para obtener nodos de transacción adicionales, aumente el número de puerto en uno (por ejemplo, el primer nodo de transacción está en el puerto 4000).
...
![buscar azure traffic manager](./media/ethereum-poa-deployment-guide/traffic-manager-search.png)

Asigne un nombre único al perfil y seleccione el grupo de recursos que se creó durante la implementación de prueba de autoridad.

![crear traffic manager](./media/ethereum-poa-deployment-guide/traffic-manager-create.png)

Una vez implementado, seleccione la instancia del grupo de recursos. El nombre DNS para acceder al administrador de tráfico puede encontrarse en la pestaña Información general

![Localizar DNS de Traffic Manager](./media/ethereum-poa-deployment-guide/traffic-manager-dns.png)

Seleccione la pestaña Puntos de conexión y haga clic en el botón Agregar. Después, cambie el tipo de recurso de destino a la dirección IP pública. Luego, seleccione la dirección IP pública del equilibrador de carga de la primera\' región.

![Enrutamiento de Traffic Manager](./media/ethereum-poa-deployment-guide/traffic-manager-routing.png)

Repita para cada región de la red implementada. Una vez que los puntos de conexión estén en el estado \"Habilitado\", se cargarán automáticamente y la región se equilibrará en el nombre DNS del administrador de tráfico. Ahora puede usar este nombre DNS en lugar del parámetro \[CONSORTIUM\_DATA\_URL\] en otros pasos del documento.

## <a name="data-api"></a>API de datos

Cada miembro del consorcio hospeda la información necesaria para que otros usuarios se conecten a la red. El miembro existente proporcionará el parámetro [CONSORTIUM_DATA_URL] antes de la implementación del miembro. Tras la implementación, un miembro de unión recuperará la información de la interfaz JSON en el punto de conexión siguiente:

`<CONSORTIUM_DATA_URL>/networkinfo`

La respuesta contendrá información útil para miembros de unión (bloque génesis, contrato ABI de conjunto de validadores, nodos de arranque) así como información útil para el miembro existente (direcciones del validador). Se recomienda el uso de esta normalización para extender el consorcio a través de proveedores de nube. Esta API devolverá una respuesta con formato JSON con la estructura siguiente:
```json
{
  "$id": "", 
  "type": "object", 
...
### <a name="debug-a-smart-contract-truffle-develop"></a>Depurar un contrato inteligente (Truffle Develop)
...
Esto funciona bien como un mecanismo para proporcionar capacidad de auditoría a los reguladores de la red. Para lograr esto simplemente siga el paso 2 de la sección Crecimiento del consorcio.
...
### <a name="are-public-ip-deployments-compatible-with-private-network-deployments"></a>¿Son compatibles con las implementaciones de dirección IP pública con las implementaciones de red privada?
