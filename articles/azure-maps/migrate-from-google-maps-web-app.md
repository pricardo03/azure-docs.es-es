...
En la siguiente tabla se indican las características clave de API del SDK de JavaScript para Google Maps V3 y la compatibilidad de una API similar en el SDK web de Azure Maps.
...
Las siguientes son algunas de las diferencias clave entre los SDK web de Google Maps y Azure Maps que se deben tener en cuenta:

- Además de proporcionar un punto de conexión hospedado para acceder al SDK web para Azure Maps, también está disponible un paquete NPM para insertar el SDK web en aplicaciones, si lo prefiere. Consulte esta [documentación](how-to-use-map-control.md) para obtener más información. Este paquete también incluye las definiciones de TypeScript.
- Después de crear una instancia de la clase Map en Azure Maps, el código debe esperar a que se active el evento `ready` o `load` del mapa antes de interactuar con el mapa. Esto garantizará que todos los recursos del mapa se hayan cargado y estén listos para su acceso.
...
- Las coordenadas de Google Maps se conocen como "latitud, longitud", mientras que Azure Maps usa "longitud, latitud". Esto se alinea con el estándar `[x, y]`, que siguen la mayoría de las plataformas de GIS.
- Las formas del SDK web para Azure Maps se basan en el esquema GeoJSON. Las clases auxiliares se exponen a través del [espacio de nombres *atlas.data*](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.data?view=azure-iot-typescript-latest). También existe la clase [*atlas.Shape*](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.shape) que se puede usar para encapsular objetos GeoJSON y facilitar su actualización y mantenimiento de un modo enlazable de datos.
- Las coordenadas se definen en Azure Maps como objetos de posición que se pueden especificar como una matriz de números simple con el formato `[longitude, latitude]` o el nuevo elemento atlas.data.Position(longitud, latitud).
    > [!TIP]
    > La clase Position tiene un método auxiliar estático para importar las coordenadas que están en formato "latitud, longitud". El método [atlas.data.Position.fromLatLng](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.data.position?view=azure-iot-typescript-latest) puede reemplazar a menudo el método `new google.maps.LatLng` en el código de Google Maps.
...
- Google Maps requiere que se especifique una clave de cuenta en la referencia de script de la API. Las credenciales de autenticación para Azure Maps se especifican como opciones de la clase map. Puede ser una clave de suscripción o información de Azure Active Directory.
...
- Las coordenadas se definen en Azure Maps como objetos de posición que se pueden especificar como una matriz de números simple con el formato `[longitude, latitude]`.
- El nivel de zoom en Azure Maps es un nivel inferior al del ejemplo de Google Maps debido a la diferencia en los tamaños del sistema de mosaicos de las plataformas.
...
> [!NOTE]
> A diferencia de Google Maps, Azure Maps no necesita que se especifique un centro y un nivel de zoom iniciales al cargar el mapa. Si no se proporciona esta información al cargar el mapa, este intentará determinar en qué ciudad se encuentra el usuario, y centrará y aplicará el zoom del mapa allí.
...
Azure Maps proporciona dos formas diferentes de establecer el idioma y la vista regional del mapa. La primera opción es agregar esta información al espacio de nombres *atlas* global, lo que provocará que estos valores sean los predeterminados en todas las instancias de control de mapa de la aplicación. El código siguiente establece el idioma en francés ("fr-FR") y la vista regional en "auto":
...
Los mapas dinámicos en Azure y Google Maps se pueden mover mediante programación a nuevas ubicaciones geográficas mediante una llamada a las funciones adecuadas en JavaScript. En los siguientes ejemplos se muestra cómo hacer que el mapa muestre las imágenes aéreas de satélite, centre el mapa sobre una ubicación con las coordenadas (longitud: -111,0225, latitud: 35,0272) y cambie el nivel de zoom a 15 en Google Maps.
...
El control de mapa de Google Maps puede moverse mediante programación con el método `setOptions`, lo que permite especificar el centro del mapa y un nivel de zoom.
...
En Azure Maps, la posición del mapa se puede cambiar mediante programación con el método `setCamera` del mapa y el estilo de mapa se puede cambiar con el método `setStyle`. Tenga en cuenta que las coordenadas en Azure Maps tienen el formato "longitud, latitud" y se resta uno al valor de nivel de zoom.
...
En Azure Maps hay varias maneras en las que se pueden representar los datos de punto en el mapa;
...
En Azure Maps, se pueden usar los marcadores HTML para mostrar un punto en el mapa y se recomiendan solo para las aplicaciones sencillas que únicamente necesitan mostrar un pequeño número de puntos en el mapa. Para usar un marcador HTML, basta con crear una instancia de la clase `atlas.HtmlMarker`, establecer las opciones de texto y posición y agregar el marcador al mapa mediante el método `map.markers.add`.
...
Los polígonos se usan para representar un área en el mapa. Azure Maps y Google Maps proporcionan una compatibilidad muy similar para los polígonos. En los siguientes ejemplos se muestra cómo crear un polígono que forme un triángulo basado en la coordenada del centro del mapa.
...
Se puede mostrar información adicional para una entidad en el mapa como una clase `google.maps.InfoWindow` en Google Maps, en Azure Maps se puede lograr mediante la clase `atlas.Popup`. En los siguientes ejemplos se agrega un marcador al mapa y, cuando se hace clic en él, se muestra una ventana de información o un elemento emergente.
...
Google Maps admite la carga y la aplicación dinámica de estilo a los datos GeoJSON mediante la clase `google.maps.Data`. La funcionalidad de esta clase se alinea mucho más con el estilo controlado por datos de Azure Maps. Una diferencia clave es que con Google Maps se especifica una función de devolución de llamada y la lógica de negocios para aplicar estilos a cada característica que se procesa individualmente en el subproceso de interfaz de usuario. En Azure Maps, las capas admiten la especificación de expresiones controladas por datos como opciones de estilo. Estas expresiones se procesan en tiempo de representación en un subproceso independiente, proporcionan un aumento del rendimiento de la representación y permiten que conjuntos de datos mayores se representen con más rapidez.

En los siguientes ejemplos se carga una fuente GeoJSON de todos los terremotos en los últimos siete días de los USG y se representan como círculos a escala en el mapa. El color y la escala de cada círculo se basan en la magnitud de cada terremoto que está almacenada en la propiedad `"mag"` de cada característica del conjunto de datos. Si la magnitud es mayor o igual que cinco, el círculo será rojo, si es mayor o igual que tres, pero menor que cinco, el círculo será naranja y, si es inferior a tres, el círculo será verde. El radio de cada círculo será el valor exponencial de la magnitud multiplicado por 0,1.

**Antes: Google Maps**

En Google Maps, se puede especificar una función de devolución de llamada única en el método `map.data.setStyle` que se usará para aplicar la lógica de negocios a cada característica que se carga desde la fuente GeoJSON a través del método `map.data.loadGeoJson`.
...
En Google Maps, los marcadores se pueden agrupar en clústeres mediante la carga en la biblioteca MarkerClusterer. Los iconos de clúster se limitan a ser imágenes que tienen los números del uno al cinco como su nombre y se hospedan en el mismo directorio.
...
Al representar los datos en clúster en el mapa, a menudo es más fácil usar dos o más capas. En el ejemplo siguiente se usan tres capas, una capa de burbujas para dibujar círculos de colores escalados según el tamaño de los clústeres, una capa de símbolos para representar el tamaño de clúster como texto y una segunda capa de símbolos para representar los puntos no agrupados en clústeres. Hay muchas otras maneras de representar los datos agrupados en clústeres en Azure Maps resaltadas en la documentación sobre los [datos de puntos de clúster](clustering-point-data-web-sdk.md).
...
En los siguientes ejemplos se carga una fuente GeoJSON de todos los terremotos del último mes desde los USG y se representan como un mapa térmico ponderado en el que se usa la propiedad `"mag"` como ponderación.

**Antes: Google Maps**

En Google Maps, para crear un mapa térmico es necesario cargar la biblioteca "visualization" agregando `&libraries=visualization` a la dirección URL del script de la API. La capa de mapa térmico de Google Maps no es compatible directamente con los datos GeoJSON y, en su lugar, los datos se deben descargar y convertir primero en una matriz de puntos de datos ponderados.
...
Las capas de mosaicos, también conocidas como superposiciones de imágenes en Google Maps, permiten superponer imágenes grandes que se han dividido en imágenes en mosaico más pequeñas y que se alinean con el sistema de mosaicos de los mapas. Se trata de una manera común de superponer imágenes grandes o conjuntos de datos muy grandes.
...
En el siguiente apéndice se proporciona una asignación de referencias cruzadas de las clases que se usan con más frecuencia en Google Maps V3 a sus equivalentes del SDK Web de Azure Maps.
...
El SDK web de Azure Maps incluye un [módulo de servicios](how-to-use-services-module.md) que se puede cargar por separado. Este módulo encapsula los servicios REST de Azure Maps con una API web y se puede usar en aplicaciones JavaScript, TypeScript y Node.js.
