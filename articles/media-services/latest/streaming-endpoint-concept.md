---
title: Puntos de conexión de streaming (Origen) en Azure Media Services | Microsoft Docs
description: En Microsoft Azure Media Services, un punto de conexión de streaming (Origen) representa un servicio de empaquetado dinámico y streaming que puede entregar contenido directamente a una aplicación de reproducción de cliente o a una instancia de Content Delivery Network (CDN) para la distribución posterior.
...
> [!NOTE]
> Para los clientes que desean entregar contenido a grandes audiencias de Internet, se recomienda habilitar CDN en el punto de conexión de streaming.
...
- `accessControl`: se usa para configurar las siguientes opciones de seguridad para este punto de conexión de streaming: Claves de autenticación de encabezado de firma de Akamai y direcciones IP que puedan conectarse a este punto de conexión.<br />Esta propiedad solo se puede establecer cuando la propiedad `cdnEnabled` se establece en false.
- `cdnEnabled`: indica si la integración de Azure CDN para este punto de conexión de streaming está habilitada (está deshabilitada de forma predeterminada). Si establece la propiedad `cdnEnabled` en true, se deshabilitarán las siguientes configuraciones: `customHostNames` y `accessControl`.
  
    No todos los centros de datos admiten la integración de Azure CDN. Para comprobar si su centro de datos tiene disponible la integración de la red Azure CDN, haga lo siguiente:
 
  - Intente establecer el valor de `cdnEnabled` en true.
  - Compruebe el resultado devuelto de `HTTP Error Code 412` (error de condición previa) con el mensaje "Streaming endpoint CdnEnabled property cannot be set to true as CDN capability is not available in the current region" (La propiedad CdnEnabled del punto de conexión de streaming no se puede establecer en true ya que la funcionalidad de red CDN no está disponible en la región actual). 
...
- `resourceState` -
    ...
    - Stopped (Detenido): el estado inicial de un punto de conexión de streaming después de su creación.
    - Starting (Iniciando): está realizando la transición al estado En ejecución.
    - Running (En ejecución): capaz de hacer streaming del contenido a clientes.
    - Scaling (Escalando): las unidades de escalado se están aumentando o reduciendo.
    - Stopping (Deteniendo): está realizando la transición al estado Detenido.
    - Deleting (Eliminando): se va a eliminar.
    
- La propiedad `scaleUnits` proporciona capacidad de salida dedicada que se puede adquirir en incrementos de 200 Mbps. Si tiene que pasar a un tipo **premium**, ajuste la propiedad `scaleUnits`.
...
### <a name="determine-if-dns-change-has-been-made"></a>Determinar si se ha realizado el cambio de DNS
