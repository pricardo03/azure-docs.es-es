...
1. [Instale el entorno en tiempo de ejecución de Azure IoT Edge](./how-to-install-iot-edge-windows-with-windows.md) en el dispositivo Windows que vaya a usar como puerta de enlace transparente.
...
1. Obtenga OpenSSL para Windows. Existen muchas maneras de instalar OpenSSL:
   ...
   * Descargue el código fuente de OpenSSL y compile los archivos binarios en su máquina por sí mismo o a través de [vcpkg](https://github.com/Microsoft/vcpkg). Las instrucciones que aparecen a continuación usan vcpkg para descargar el código fuente, compilar e instalar OpenSSL en el equipo Windows con pasos fáciles.
      ...
      1. Después de instalar vcpkg, desde un símbolo del sistema de Powershell, ejecute el siguiente comando para instalar el paquete de OpenSSL para Windows x64. Esta instalación suele tardar, aproximadamente, 5 minutos en completarse.
...
1. Copie los archivos siguientes de $WRKDIR en cualquier parte de su dispositivo de Edge; haremos referencia a esto como $CERTDIR. Si ha generado los certificados en el dispositivo Edge, omita este paso.
...
* Windows: este es un ejemplo de cómo instalar un certificado de entidad de certificación en un host de Windows.
  1. En el menú de inicio, escriba "Administrar certificados de equipo". Esto debería iniciar una utilidad denominada `certlm`.
  2. Navegue hasta **Certificados - Equipo Local** > **Certificado raíz de confianza** > **Certificados** > Haga clic con el botón derecho > **Todas las tareas** > **Importar** para iniciar el asistente para importar certificados.
  3. Siga los pasos tal como se indica e importe el archivo de certificado $CERTDIR/certs/azure-iot-test-only.root.ca.cert.pem.
  4. Cuando haya completado, verá el mensaje "Importación correcta".
