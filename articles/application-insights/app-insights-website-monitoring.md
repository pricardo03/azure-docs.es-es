...
Esta guía de inicio rápido ayudará a través de la adición del [SDK de JavaScript de Application Insight de código abierto](https://github.com/Microsoft/ApplicationInsights-JS), que le permite comprender la experiencia del cliente o explorador para los visitantes del sitio web.
...
2. Haga clic en el icono ![Mapa de aplicaciones](./media/app-insights-nodejs-quick-start/006.png) **Analytics**.  Se abrirá **Analytics**, que proporciona un lenguaje de consulta avanzado para analizar todos los datos recopilados por Application Insights. Para ver los datos relacionados con las solicitudes de explorador del lado cliente, ejecute la siguiente consulta:
   ...
   ![Gráfico de Analytics con las solicitudes de usuario durante un período de tiempo](./media/app-insights-website-monitoring/analytics-query.png)

3. Vuelva a la página **Introducción**. Haga clic en **Explorador** en el encabezado **Investigar** y, después, seleccione **Rendimiento**. Aquí encontrará las métricas relacionadas con el rendimiento del sitio web. También hay una vista para analizar los errores y excepciones en su sitio web correspondiente. Puede hacer clic en **Ejemplos** para profundizar en los detalles de cada transacción. Desde aquí, puede tener acceso a la experiencia de los [detalles de la transacción de un extremo a otro](app-insights-transaction-diagnostics.md).

   ![Gráfico de las métricas del servidor](./media/app-insights-website-monitoring/browser-performance.png)

4. Para comenzar a explorar las [herramientas de análisis del comportamiento del usuario](app-insights-usage-overview.md), en el menú principal de Application Insights seleccione [**Usuarios**](app-insights-usage-segmentation.md) bajo el encabezado **Uso**. Como estamos probando desde una sola máquina, solo se verán los datos de un usuario. Para un sitio web en vivo, la distribución de los usuarios podría tener el aspecto siguiente:

     ![Gráfico de usuario](./media/app-insights-website-monitoring/usage-users.png)

5. Si se hubiera instrumentado un sitio web más complejo con múltiples páginas, otra herramienta útil es [**Flujos de usuario**](app-insights-usage-flows.md). Con **Flujos de usuario** puede hacer un seguimiento de la ruta que siguen los visitantes a través de las distintas partes del sitio web.

  ![Visualización de flujos de usuario](./media/app-insights-website-monitoring/user-flows.png)
