...
> [!NOTE]
> Según los comentarios recibidos de los usuarios sobre que la **pila de copia de seguridad de VM V2** es confusa con Azure Stack, le hemos cambiado el nombre a **restauración instantánea**, lo que proporciona una experiencia mejor y actualizada.
...
* Reduce los tiempos de copia de seguridad y restauración al conservarse las instantáneas junto con los discos durante siete días.
...
Depende de la actividad de la máquina virtual. En un estado estable, podemos suponer que el aumento del costo es = período de retención de la instantánea * actividad diaria por máquina virtual * costo de almacenamiento por GB.
...
El nuevo modelo no permite eliminar el punto de restauración (nivel 2), a menos que se elimine la instantánea (nivel 1). Actualmente admitimos períodos de retención de siete días para la eliminación de la instantánea (nivel 1), por lo que el período de retención del punto de restauración (nivel 2) inferior a siete días no se respeta. Se recomienda programar un período de retención del punto de restauración (nivel 2) superior a siete días.
