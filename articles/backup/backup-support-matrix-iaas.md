...
- [Matriz de compatibilidad](backup-support-matrix-mabs-dpm.md) para copia de seguridad de DOMO y del servidor de Microsoft Azure Backup
- [Matriz de compatibilidad](backup-support-matrix-mars-agent.md) para copia de seguridad con el agente de MARS
...
--- | ---
Habilitar copia de seguridad al crear una máquina virtual de Azure con Windows | Compatible para:  Windows Server 2016 (Datacenter/Datacenter Core); Windows Server 2012 R2 DataCenter; Windows Server 2008 R2 SP1
...
Copia de seguridad de discos después de migrar a discos administrados. |  Se admite.<br/><br/> La copia de seguridad seguirá funcionando. no se requiere ninguna acción.
Copia de seguridad de discos administrados después de habilitar el bloqueo del grupo de recursos | No compatible.<br/><br/> Backup no puede eliminar los puntos de recursos anteriores y las copias de seguridad empezarán a generar errores cuando se alcance el límite máximo de puntos de restauración.
Modificar directiva de copia de seguridad de una máquina virtual |  Se admite.<br/><br/> Se realizará la copia de seguridad de la máquina virtual con la programación y la configuración de retención de la nueva directiva. Si la configuración de retención está extendida, los puntos de recuperación existentes se marcan y se mantienen. Si se reducen, los puntos de recuperación existentes se eliminarán en el siguiente trabajo de limpieza y, después, se eliminarán por completo.
...
Copias de seguridad por día (mediante la extensión de máquina virtual de Azure) | Una copia de seguridad programada por día.<br/><br/> Puede realizar hasta cuatro copias de seguridad a petición por día.
...
Ajuste automático del reloj | No compatible.<br/><br/> Azure Backup no tiene en cuenta el ajuste automático del reloj para los cambios de horario de verano para realizar copias de seguridad de máquinas virtuales.<br/><br/>  Modifique la directiva de forma manual según sea necesario.
[Características de seguridad para copias de seguridad híbridas](https://docs.microsoft.com/azure/backup/backup-azure-security-feature) |  No se pueden deshabilitar las características.



## <a name="operating-system-support-windows"></a>Compatibilidad con sistema operativo (Windows)

En la tabla se resumen los sistemas operativos compatibles para realizar copias de seguridad de máquinas virtuales de Azure con Windows.

**Escenario** | **SO compatible** 
--- | ---
Copia de seguridad con la extensión del agente de máquina virtual de Azure | Cliente Windows: No compatible<br/><br/> Windows Server: Windows Server 2008 R2 o versiones posteriores 
...
Para las copias de seguridad de máquinas virtuales de Azure con Linux, Azure Backup admite la [lista de distribuciones aprobadas por Azure](https://docs.microsoft.com/azure/virtual-machines/linux/endorsed-distros) para Linux. Observe lo siguiente:
...
--- | --- 
Puntos de recuperación máximos por instancia protegida (máquina/carga de trabajo) | 9.999
...
En la tabla se resume la compatibilidad con copias de seguridad durante las tareas de administración de máquinas virtuales, como agregar o reemplazar discos de máquina virtual.
...
Restaurar en cuenta de almacenamiento mediante el almacenamiento con redundancia de zona | No compatible
...
Restaurar copia de seguridad de máquinas virtuales no administradas después de actualizar a discos administrados |  Se admite.<br/><br/> Puede restaurar discos y después crear una máquina virtual administrada.
...
Restaurar una máquina virtual en una red virtual distinta |   Se admite.<br/><br/> La red virtual debe estar en la misma suscripción y región.
...
Copia de seguridad de máquinas virtuales en [conjuntos de disponibilidad](https://docs.microsoft.com/azure/virtual-machines/windows/regions-and-availability#availability-sets) |  Se admite.<br/><br/> No se puede restaurar una máquina virtual en un conjunto disponible con la opción para crear rápidamente una máquina virtual. En su lugar, al restaurar la máquina virtual, necesita restaurar el disco y usarlo para implementar una máquina virtual, o bien restaurar un disco y usarlo para reemplazar un disco existente. 
...
Copia de seguridad de máquinas virtuales implementadas con la [ventaja de uso híbrido (HUB)](https://docs.microsoft.com/azure/virtual-machines/windows/hybrid-use-benefit-licensing) |  Se admite.
...
Copia de seguridad de máquinas virtuales implementadas desde [Azure Marketplace](https://azuremarketplace.microsoft.com/en-us/marketplace/apps?filters=virtual-machine-images)<br/><br/> (Publicado por Microsoft y terceros) |    Se admite.<br/><br/> La máquina virtual debe ejecutar un sistema operativo compatible.<br/><br/> Al recuperar archivos en la máquina virtual, puede restaurar solo en un sistema operativo compatible (no en un sistema operativo anterior ni posterior).
Copia de seguridad de máquinas virtuales implementadas desde una imagen personalizada (terceros) |     Se admite.<br/><br/> La máquina virtual debe ejecutar un sistema operativo compatible.<br/><br/> Al recuperar archivos en la máquina virtual, puede restaurar solo en un sistema operativo compatible (no en un sistema operativo anterior ni posterior).
Copia de seguridad de máquinas virtuales migradas a Azure   |  Se admite.<br/><br/> Para realizar copias de seguridad de la máquina virtual, el agente de máquina virtual debe estar instalado en la máquina migrada. 
...
Tamaño del disco de datos | El disco individual puede tener hasta 4095 GB.<br/><br/> Si ejecuta la última versión de copia de seguridad de máquina virtual de Azure (lo que se conoce como restauración instantánea), se admiten tamaños de disco de hasta 4 TB. [Más información](backup-instant-restore-capability.md).
Tipo de almacenamiento | HDD estándar, SSD estándar y SSD premium <br/><br/> SSD estándar se admite si ejecuta la última versión de copia de seguridad de máquina virtual de Azure (lo que se conoce como restauración instantánea). [Más información](backup-instant-restore-capability.md).
Discos administrados | Compatible
Discos cifrados |  Se admite.<br/><br/> Se puede realizar una copia de seguridad (con o sin la aplicación Azure AD) de las máquinas virtuales de Azure que tengan habilitado Azure Disk Encryption (ADE).<br/><br/> Las máquinas virtuales cifradas no se pueden recuperar a nivel de archivo o carpeta. Necesita recuperar la máquina virtual completa.<br/><br/> Puede habilitar el cifrado en máquinas virtuales que ya estén protegidas con Azure Backup.
...
**Componente** | **Soporte técnico**
--- | ---
Número de NIC | Hasta el número máximo de NIC admitidas por un tamaño específico de máquina virtual de Azure.<br/><br/> Las NIC se crean cuando se crea la máquina virtual durante el proceso de restauración.<br/><br/> El número de NIC de la máquina virtual restaurada refleja el número de NIC de la máquina virtual cuando se habilita la protección. La eliminación de NIC después de habilitar esta opción no afecta al recuento.
Equilibrador de carga interno y externo |    Se admite. <br/></br> [Obtenga más información](backup-azure-arm-restore-vms.md#restore-vms-with-special-network-configurations) sobre cómo restaurar máquinas virtuales con una configuración de red especial.
Varias direcciones IP reservadas |   Se admite. <br/></br> [Obtenga más información](backup-azure-arm-restore-vms.md#restore-vms-with-special-network-configurations) sobre cómo restaurar máquinas virtuales con una configuración de red especial.
Máquinas virtuales con varios adaptadores de red  |  Se admite. <br/></br> [Obtenga más información](backup-azure-arm-restore-vms.md#restore-vms-with-special-network-configurations) sobre cómo restaurar máquinas virtuales con una configuración de red especial.
Máquinas virtuales con direcciones IP públicas    |  Se admite.<br/><br/> Deberá asociar una dirección IP pública existente a la NIC, o crear una dirección y asociarla a la NIC después de realizar la restauración.
Grupo de seguridad de red (NSG) en la NIC o la subred. |    Se admite.
Dirección IP reservada (estática) |  No se admite.<br/><br/> No puede realizar una copia de seguridad de una máquina virtual con una dirección IP reservada y ningún punto de conexión definido.
...
Traffic Manager | Compatible<br/><br/>. Si la máquina virtual de copia de seguridad está en Traffic Manager, deberá agregar manualmente la máquina virtual restaurada al mismo Traffic Manager. 
Azure DNS |  Se admite.
...
Puntos de conexión de servicio de red virtual  |  Se admite.<br/><br/> La configuración del firewall y de la cuenta de almacenamiento de la red virtual deben permitir el acceso desde todas las redes. 



## <a name="vm-securityencryption-support"></a>Compatibilidad con cifrado y seguridad de máquina virtual
