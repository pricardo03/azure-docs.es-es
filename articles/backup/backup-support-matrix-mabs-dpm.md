...
ms.locfileid: "56431029"
---
# <a name="support-matrix-for-backup-with-microsoft-azure-backup-serversystem-center-dpm"></a>Matriz de compatibilidad para copia de seguridad con Microsoft Azure Backup Server y System Center DPM
...
## <a name="about-mabsdpm"></a>Acerca de MABS/DPM
...
Microsoft Azure Backup Server (MABS) es un producto de servidor que se puede usar para realizar una copia de seguridad de servidores físicos locales, máquinas virtuales (VM) y aplicaciones que se ejecutan en ellos y de máquinas virtuales de Azure.

- MABS se basa en System Center Data Protection Manager (DPM) y proporciona una funcionalidad similar con un par de diferencias:
...
- Puede descargar el servidor de MABS del [Centro de descarga de Microsoft](https://www.microsoft.com/en-us/download/details.aspx?id=57520). Se puede ejecutar de forma local o en una máquina virtual de Azure en Azure.
...
## <a name="mabsdpm-backup"></a>Copia de seguridad con DPM/MABS

La copia de seguridad con MABS/DPM y Azure Backup funciona como sigue:

1. Agente de protección de DPM/MABS instalado en cada máquina de la que se realizará una copia de seguridad. 
...
--- | --- | ---
**Copia de seguridad de cargas de trabajo/máquinas locales** | El agente de protección de DPM/MABS se ejecuta en las máquinas de las que desea realizar una copia de seguridad<br/><br/> El agente de MARS en el servidor de DPM/MABS | MABS/DPM debe ejecutarse en el entorno local
**Copia de seguridad de cargas de trabajo/máquinas virtuales de Azure** | Agente de protección de DPM/MABS en la máquina protegida<br/><br/> Agente de MARS en DPM/MABS | DPM/MABS debe ejecutarse en una máquina virtual de Azure.

## <a name="supported-deployments"></a>Implementaciones admitidas

DPM/MABS puede implementarse como se resume en la tabla.
...
Azure Backup puede realizar una copia de seguridad de MABS/DPM en cualquiera de los siguientes sistemas operativos. Los sistemas operativos deben ejecutar los últimos Service Pack y actualizaciones.
...
**MABS local** | Sistemas operativos compatibles de 64 bits:<br/><br/> - MABS v3 y versiones posteriores: Windows Server 2019 (Standard, Datacenter, Essentials) <br/><br/> MABS v2 y versiones posteriores: Windows Server 2016 (Standard, Datacenter, Essentials)<br/><br/> Todas las versiones de MABS: Windows Server 2012 R2/2012 (Standard, Datacenter, Foundation)<br/><br/>Todas las versiones de MABS: Windows Storage Server 2012 R2/2012 (Standard/Workgroup)
...
**Problema** | **Detalles**
--- | ---
**Instalación** | DPM/MABS debe instalarse en una máquina de propósito único.<br/><br/> No instale DPM/MABS en un controlador de dominio, en una máquina con la instalación del rol de servidor de aplicaciones, en una máquina que ejecuta Exchange Server o System Center Operations Manager o en un nodo de clúster.<br/><br/> [Revise](https://docs.microsoft.com/system-center/dpm/prepare-environment-for-dpm?view=sc-dpm-1807#dpm-server) todos los requisitos del sistema DPM.
**Dominio** | El servidor DPM/MABS debe estar unido a un dominio. En primero lugar, instale DPM/MABS y, después, únalo a un dominio. Mueva DPM/MABS a un dominio nuevo después de que no se admita la implementación.
...
**Componente** | **Detalles**
--- | --- 
**MABS en máquinas virtuales de Azure Stack** | Tamaño mínimo de A2. Se recomienda que empiece con una imagen de Windows Server 2012 R2 o Windows Server 2016 de Azure Marketplace.<br/><br/> No debe instalar ningún otro elemento en la máquina virtual de MABS.
**Almacenamiento de MABS** | Use una cuenta de almacenamiento independiente para la máquina virtual de MAB. El agente de MARS en MABS necesita almacenamiento temporal para una ubicación de caché, así como para hospedar la restauración de datos desde la nube.
**Grupo de almacenamiento de MABS** | El tamaño del grupo de almacenamiento de MABS lo determinan el número y tamaño de los discos conectados a la máquina virtual de MABS. Cada tamaño de máquina virtual de Azure Stack tiene un número máximo de discos. Por ejemplo, A2 son cuatro discos.
**Retención de MABS** | No debe conservar los datos de copia de seguridad de los discos locales de MABS durante más de cinco días.
**Escalado vertical de MABS** | Para escalar verticalmente la implementación, puede aumentar el tamaño de la máquina virtual de MABS. Por ejemplo, de la serie A a la D.<br/><br/> También puede asegurarse de descargar los datos con copia de seguridad en Azure regularmente y, si es necesario, puede implementar servidores de MABS adicionales.
...
**Copia de seguridad compatible** | Los sistemas operativos compatibles para las máquinas virtuales de las que desea realizar una copia de seguridad son:<br/><br/> - Canal semianual de Windows Server (Datacenter/Enterprise/Standard)<br/><br/> Windows Server 2016/2012 R2/2012/2008 R2.
**Compatibilidad de SQL para máquinas virtuales de Azure Stack** | Realice una copia de seguridad de SQL Server 2016/2014/2012 SP1.<br/><br/> Realice una copia de seguridad y recuperación de la base de datos.
**Compatibilidad de SharePoint para máquinas virtuales de Azure Stack** | SharePoint 2016/2013/2010.<br/><br/> Realice una copia de seguridad y recuperación de granjas, bases de datos, front-end y servidores web.
**Requisitos de red para máquinas virtuales con copia de seguridad** | Todas las máquinas virtuales de carga de trabajo de Azure Stack deben estar en la misma red virtual y pertenecer a la misma suscripción.
...
Conectado | Expirada/desaprovisionada | Ninguna copia de seguridad en disco o en Azure.<br/><br/> Si la suscripción ha expirado, puede restaurar desde el disco o Azure.<br/><br/> Si la suscripción se ha retirado, puede restaurar desde el disco o Azure. Los puntos de recuperación de Azure se han eliminado.
...
No hay conectividad durante más de 15 días | Expirada/desaprovisionada | Ninguna copia de seguridad en disco o en Azure.<br/><br/> Si la suscripción ha expirado, puede restaurar desde el disco o Azure.<br/><br/> Si la suscripción se ha retirado, puede restaurar desde el disco o Azure. Los puntos de recuperación de Azure se han eliminado.



## <a name="dpmmabs-storage-support"></a>Compatibilidad con almacenamiento en DPM/MABS

Los datos con copia de seguridad en DPM/MABS se almacenan en el almacenamiento en disco local. 

**Storage** | **Detalles**
--- | ---
**MBS** | Modern Backup Storage (MBS) se admite desde DPM 2016/MABS v2 y versiones posteriores. No está disponible para MABS v1.
**Almacenamiento de MABS en máquina virtual de Azure** | Los datos se almacenan en discos de Azure conectados a la máquina virtual de DPM/MABS y se administran en DPM/MABS. El número de discos que se pueden usar para el grupo de almacenamiento de DPM/MABS está limitado por el tamaño de la máquina virtual.<br/><br/> VM A2: 4 discos; VM A3: 8 discos; VM A4: 16 discos con un tamaño máximo de 1 TB para cada disco. Esto determina el grupo de almacenamiento de copia de seguridad total disponible.<br/><br/> La cantidad de datos de los que puede realizar copias de seguridad depende del número y tamaño de los discos conectados.
...
A partir de DPM 2016/MABS v2 (en Windows Server 2016), puede beneficiarse de Modern Backup Storage (MBS).
...
--- | --- | --- | ---
**Windows 10, 8.1, 8, 7**<br/><br/>(32/64 bits) | MABS v3, v2, V1 | Local | Volumen, recurso compartido, carpeta y archivo<br/><br/> Volúmenes desduplicados admitidos<br/><br/> Los volúmenes deben ser de al menos 1 GB y NTFS.
...
**Windows Server 2008 con SP2**<br/><br/> 64/32 bits | MABS v1, v2, v3 | Solo se admite v1 cuando se implementa MABS como una máquina física local o una máquina virtual de Hyper-V.<br/><br/> MABS v1, 2 y 3 es compatible cuando MABS se implementa como una máquina virtual de VMware.<br/><br/> No se admite para MABS en ejecución en una máquina virtual de Azure | Volumen, recurso compartido, carpeta, archivo, estado del sistema y reconstrucción completa.
**Windows Storage Server 2008** | MABS v1, v2, v3 | MABS como servidor físico local/máquina virtual de Hyper-V | No se admite para MABS en ejecución en máquinas virtuales de Azure. | Volumen, recurso compartido, carpeta, archivo, estado del sistema y reconstrucción completa.
...
**Máquinas virtuales de VMware: vCenter/vSphere ESXi 5.5/6.0/6.5** | MABS v3, v2, v1.<br/><br/> DPM 2012 R2 necesita el paquete acumulativo de actualizaciones 1 de System Center | Local | Copia de seguridad de máquinas virtuales de VMware en el almacenamiento CSV, NFS y SAN.<br/><br/> Recuperación de máquina virtual completa.<br/><br/> Copia de seguridad en Windows/Linux.<br/><br/> Recuperación a nivel de elemento de carpetas y archivos solo para máquinas virtuales Windows.<br/><br/> VMware vApps no compatible.<br/><br/> La recuperación de máquinas virtuales Linux es para toda la máquina.
...
- [Configuración](https://docs.microsoft.com/system-center/dpm/install-dpm?view=sc-dpm-180)
