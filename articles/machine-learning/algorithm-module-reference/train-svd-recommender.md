...
description: Aprenda a usar el módulo Train SVD Recommender (Entrenar recomendador de SVD) de Azure Machine Learning Service para entrenar un recomendador bayesiano mediante el algoritmo SVD.
...
En este artículo se describe cómo usar el módulo Train SVD Recommender (Entrenar recomendador de SVD) del diseñador de Azure Machine Learning (versión preliminar). Use este módulo para entrenar un modelo de recomendación basado en el algoritmo de descomposición en valores singulares (SVD).  

El módulo Train SVD Recommender (Entrenar recomendador de SVD) lee un conjunto de datos de tripletas usuario-elemento-clasificación. Devuelve un recomendador SVD entrenado. Después, puede usar el modelo entrenado para predecir las clasificaciones o generar recomendaciones con el uso del módulo [ de puntuación del recomendador SVD](score-svd-recommender.md).  
...
+ Un enfoque **basado en el contenido** hace uso de las características tanto para usuarios como para elementos. Los usuarios se pueden describir mediante propiedades como la edad y el género. Los elementos se pueden describir mediante propiedades como el creador y el fabricante. En los sitios de encuentros sociales se pueden encontrar ejemplos típicos de sistemas de recomendación basados en contenido. 
...
1.  Agregue el módulo Train SVD Recommender (Entrenar recomendador de SVD) a la canalización del diseñador y conéctelo a los datos de aprendizaje.  
...
3.  **Number of recommendation algorithm iterations** (Número de iteraciones recomendadas del algoritmo) indica el número de veces que el algoritmo debe procesar los datos de entrada. Cuanto mayor sea este número, más precisas serán las predicciones. Sin embargo, un número mayor también significa que el aprendizaje será más lento. El valor predeterminado es 30.
