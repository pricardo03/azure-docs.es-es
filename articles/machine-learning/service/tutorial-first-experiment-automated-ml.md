...
ms.locfileid: "68445082"
---
# <a name="tutorial-train-and-deploy-a-classification-model-with-automated-machine-learning-in-the-azure-portal-preview"></a>Tutorial: Entrenamiento e implementación de un modelo de clasificación con aprendizaje automático automatizado en Azure Portal (versión preliminar)
...
* El archivo de datos **bankmarketing_train.csv**. Descárguela [aquí](https://automlsamplenotebookdata.blob.core.windows.net/automl-sample-notebook-data/bankmarketing_train.csv).
...
1. Navegue hasta el panel izquierdo del área de trabajo. Seleccione **Aprendizaje automático automatizado**  en la sección **Creación (versión preliminar)** .
...
1. Seleccione **Create a new compute** (Crear un proceso) y configure el contexto del proceso de este experimento.
    ...
    Configuración adicional| *Min node* (Nodos mín.): 1. Para habilitar la generación de perfiles de datos, debe tener 1 o más nodos. <br> *Max node* (Nodos máx.): 6. 

    Para iniciar la creación del nuevo proceso, seleccione **Crear**. Esta operación puede tardar unos segundos. 

    Una vez finalizada la creación, seleccione el nuevo proceso en la lista desplegable y, después, seleccione **Siguiente**.

1. En este tutorial, se usa la cuenta de almacenamiento y el contenedor predeterminados que se han creado con el nuevo proceso. Así se rellena automáticamente el formulario.

1. Seleccione **Upload** (Cargar) para elegir el archivo **bankmarketing_train.csv** en el equipo local y cárguelo en el contenedor predeterminado. La versión preliminar pública solo admite cargas de archivos locales y cuentas de Azure Blob Storage. Cuando se haya completado la carga, seleccione el archivo en la lista. 
...
1. La pestaña **Preview** (Versión preliminar) permite configurar más datos de este experimento.
    ...
    En la pestaña Preview (Versión preliminar), indique que los datos incluyen encabezados. De forma predeterminada, el servicio incluye todas las características (columnas) para el entrenamiento. En este ejemplo, desplácese hacia la derecha y elija **Ignore** (Omitir) para ignorar la característica **day_of_week**.

    ![Configuración de la pestaña Preview (Versión preliminar)](media/tutorial-1st-experiment-automated-ml/preview-tab-config.gif)


    >[!NOTE]
    > La generación de perfiles de datos no está disponible en aquellos procesos que tienen 0 nodos mínimos.

1. Seleccione **Classification** (Clasificación) como tarea de predicción.

1. Seleccione **y** como columna de destino, la columna en la que desea realizar las predicciones. Esta columna indica si el cliente ha suscrito un depósito a plazo o no.

1. Expanda **Advanced Settings** (Configuración avanzada) y rellene los campos como se indica a continuación.
    ...
    Preprocessing (Preprocesamiento)| Habilite el preprocesamiento que realiza el aprendizaje automático. Aquí se incluyen la limpieza, preparación y transformación automáticas de los datos para generar características sintéticas.
   ...
   >[!NOTE]
   > Para este experimento, no se establece una métrica o un umbral máximo de iteraciones, y no se bloquea ningún algoritmo, con el fin de que se puedan probar todos los que se desee.

1. Para ejecutar el experimento, haga clic en **Start** (Iniciar).

   Cuando se inicie el experimento, verá la pantalla **Run Detail** (Detalle de ejecución) en blanco con el siguiente estado en la parte superior. El proceso de preparación del experimento tarda unos minutos. Cuando se completa el proceso de preparación, el mensaje de estado cambia a **Run is Running** (Ejecución en marcha).
...
A medida que el experimento progresa, la pantalla **Run Detail** (Detalle de ejecución) actualiza el gráfico de iteración y la lista con las distintas iteraciones (modelos) que se ejecutan. La lista de iteraciones está en orden por puntuación de métrica y, de forma predeterminada, el modelo con más alta puntuación, según nuestra métrica **AUC_weighted**, está el primero de la lista.

>[!TIP]
> Los trabajos de entrenamiento pueden tardar varios minutos para que cada canalización termine de ejecutarse.
...
Para este experimento **VotingEnsemble** se considera el mejor modelo según la métrica **AUC_weighted**. Con el aprendizaje automático automatizado en Azure Portal, se puede implementar este modelo como servicio web para predecir nuevos datos con un solo clic. 
...
### <a name="delete-deployment-instance"></a>Eliminación de una instancia de implementación
