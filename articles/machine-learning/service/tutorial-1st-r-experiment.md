---
title: 'Tutorial: Primer experimento de Machine Learning con R'
titleSuffix: Azure Machine Learning
description: En este tutorial aprenderá los patrones de diseño básico de Azure Machine Learning y cómo entrenar un modelo de regresión logística con los paquetes de R azuremlsdk y caret para predecir la probabilidad de muerte en un accidente de automóvil.
...
En este tutorial aprenderá los patrones de diseño básico en Azure Machine Learning.  Entrenará e implementará un modelo de **caret** para predecir la probabilidad de muerte en un accidente de automóvil. Después de completar este tutorial tendrá el conocimiento práctico del SDK de R que le permitirá escalar verticalmente al desarrollo de experimentos y flujos de trabajo más complejos.
...
> * Carga de datos en el almacén de datos para que estén disponibles para el entrenamiento remoto
> * Creación de un recurso de proceso
> * Entrenamiento de un modelo de caret para predecir la probabilidad de muerte
...
1. Siga las [instrucciones de instalación](https://azure.github.io/azureml-sdk-for-r/articles/installation.html) para:
    + Instalar Anaconda
    + Instalar `azuremlsdk`
    + Instalar el SDK de Azure Machine Learning para Python

1. Obtenga los tres archivos de los tutoriales de [GitHub](https://github.com/Azure/azureml-sdk-for-r/tree/master/vignettes/train-and-deploy-with-caret).  Guárdelos en un directorio llamado **tutorials**.

2. Cree un área de trabajo de Azure Machine Learning y descargue su archivo de configuración según los pasos que se indican a continuación.


### <a name="create-a-workspace"></a>Crear un área de trabajo

Un área de trabajo de Azure Machine Learning es un recurso básico de la nube que se usa para experimentar, entrenar e implementar modelos de aprendizaje automático. Vincula la suscripción y el grupo de recursos de Azure con un objeto fácilmente consumido del SDK. Si ya tiene un área de trabajo de Azure Machine Learning, [vaya a la siguiente sección](#config). En caso contrario, créela ahora.

[!INCLUDE [aml-create-portal](../../../includes/aml-create-in-portal.md)]

### <a name="config"></a> Descarga del archivo config.json

1. En la parte superior de la página del área de trabajo, seleccione **Descargar config.json**.

    ![Descarga del archivo config.json](media/tutorial-1st-r-experiment/download-config.png)  

1. Agregue este archivo al directorio **tutorials**.

Ya está preparado para ejecutar el tutorial.

Para ejecutar este tutorial, se recomienda usar RStudio. En RStudio, elija File > New Project > Existing Directory (Archivo > Nuevo proyecto > Directorio existente) y seleccione el directorio **tutorials** que creó anteriormente.

> [!Important]
> El resto de este artículo incluye el mismo contenido que el del archivo **train-and-deploy-to-aci.Rmd**.  
> Si tiene experiencia con RMarkdown, no dude en usar el código de ese archivo.  También puede copiar y pegar los fragmentos de código desde allí o desde este artículo en un script de R o en la línea de comandos.
...
* Conexión a un área de trabajo para que el equipo local pueda comunicarse con los recursos remotos
...
En el tutorial se usan datos del [paquete **DAAG**](https://cran.r-project.org/package=DAAG). Instale el paquete si no lo tiene.
...
En este tutorial se usan datos del paquete **DAAG**. Este conjunto de datos incluye datos de más de 25 000 accidentes de coche en EE. UU., con variables que puede usar para predecir la probabilidad de muerte. En primer lugar, importe los datos en R, transfórmelos en una nueva trama de datos `accidents` para su análisis y expórtelos a un archivo `Rdata`.
