---
title: Desarrollo local con el Emulador de Azure Cosmos DB
...
|**Binarios**|[Descargar MSI](https://aka.ms/cosmosdb-emulator)| |**Docker**|[Docker Hub](https://hub.docker.com/r/microsoft/azure-cosmosdb-emulator/)| |**Origen de docker** | [GitHub](https://github.com/Azure/azure-cosmos-db-emulator-docker)|
...
En este momento, el Explorador de datos del emulador solo admite totalmente los clientes para la API de SQL y la API de Azure Cosmos DB para MongoDB. Los clientes para las API de Table, Graph y Cassandra no son totalmente compatibles.
...
Aunque la emulación del servicio Azure Cosmos DB es fiel, la implementación del emulador es diferente de la del servicio. Por ejemplo, el emulador utiliza componentes del sistema operativo estándar; por ejemplo, utiliza el sistema de archivos local para la persistencia y la pila del protocolo HTTPS para la conectividad. La funcionalidad que se basa en la infraestructura de Azure, como la replicación global, la latencia de milisegundos de un solo dígito para lecturas/escrituras y los niveles de coherencia ajustable, no están disponibles.
...
* En este momento, el Explorador de datos del emulador admite totalmente los clientes para la API de SQL y la API de Azure Cosmos DB para MongoDB. Los clientes para las API de Table, Graph y Cassandra no son todavía compatibles.
...
* El Emulador de Azure Cosmos DB no admite las invalidaciones de cuotas de servicio disponibles en el servicio Azure Cosmos DB (por ejemplo, los límites de tamaño del documento, el aumento del tamaño de almacenamiento de una colección con particiones).
* Como la copia del Emulador de Azure Cosmos DB puede no contener los cambios más recientes del servicio Azure Cosmos DB, debe utilizar la herramienta de [planeamiento de capacidad de Azure Cosmos DB](https://www.documentdb.com/capacityplanner) para calcular con precisión las necesidades de rendimiento de producción (RU) de la aplicación.
...
Puede descargar e instalar el emulador de Azure Cosmos DB desde el [Centro de descarga de Microsoft](https://aka.ms/cosmosdb-emulator) o puede ejecutarlo en Docker para Windows. Para obtener instrucciones acerca de cómo utilizar el emulador en Docker para Windows, consulte [Ejecución en Docker para Windows](#running-on-docker).
...
El Emulador de Azure Cosmos DB se instala de forma predeterminada en `C:\Program Files\Azure Cosmos DB Emulator`. También puede iniciar y detener el emulador desde la línea de comandos. Para más información consulte la [referencia de la herramienta de la línea de comandos](#command-line).

## <a name="start-data-explorer"></a>Inicio del Explorador de datos

Cuando se inicia el emulador de Azure Cosmos DB, el Explorador de datos de Azure Cosmos DB se abre automáticamente en el explorador. La dirección aparece como [https://localhost:8081/_explorer/index.html](https://localhost:8081/_explorer/index.html). Si cierra el explorador y desea volver a abrirlo más adelante, puede abrir la dirección URL en el explorador o iniciarlo desde el icono del Emulador de Azure Cosmos DB de la bandeja de Windows, tal como se muestra a continuación.
...
Al igual que con Azure Cosmos DB en la nube, todas las solicitudes que se realicen en el Emulador de Azure Cosmos DB deben autenticarse. El Emulador de Azure Cosmos DB es compatible con una sola cuenta fija y una clave de autenticación ya conocida para la autenticación de clave maestra. Esta cuenta y clave son las únicas credenciales que se admiten para su uso con el Emulador de Azure Cosmos DB. Son las siguientes:
...
Lo mismo que el servicio Azure Cosmos DB, el Emulador de Azure Cosmos DB solo admite la comunicación segura a través de SSL.
...
La primera vez que se habilita el acceso de red el usuario debe cerrar el emulador y eliminar el directorio de datos del emulador (C:\Users\user_name\AppData\Local\CosmosDBEmulator).

## <a name="developing-with-the-emulator"></a>Desarrollo con el emulador
Cuando tenga el emulador de Azure Cosmos DB funcionando en su escritorio, puede usar cualquier [SDK de Azure Cosmos DB](sql-api-sdk-dotnet.md) admitido o la [API REST de Azure Cosmos DB](/rest/api/cosmos-db/) para interactuar con el emulador. El emulador de Azure Cosmos DB también incluye un Explorador de datos integrado que permite crear colecciones de las API de SQL o Cosmos DB para MongoDB, así como ver y editar documentos sin escribir ningún código.
...
Si va a utilizar la [compatibilidad del protocolo de conexión de Azure Cosmos DB para MongoDB](mongodb-introduction.md), use la siguiente cadena de conexión:
...
Con el emulador de Azure Cosmos DB, puede crear de forma predeterminada hasta 25 colecciones en una única partición o una sola colección con particiones. Para más información acerca de cómo cambiar este valor, consulte la sección sobre la [configuración del valor de PartitionCount](#set-partitioncount).
...
Desde la ubicación de la instalación puede usar la línea de comandos para iniciar y detener el emulador, configurar sus opciones y realizar otras operaciones.
...
|---|---|---|---|
|[Sin argumentos] | Inicia el Emulador de Azure Cosmos DB con la configuración predeterminada. |CosmosDB.Emulator.exe| |
|[Ayuda] |Muestra la lista de argumentos de la línea de comandos admitidos.|CosmosDB.Emulator.exe /? | |
| GetStatus |Obtiene el estado del emulador de Azure Cosmos DB. El estado se indica mediante el código de salida: 1 = iniciado, 2 = en ejecución, 3 = detenido. Un código de salida negativo indica que se ha producido un error. Se produce ninguna otra salida. | CosmosDB.Emulator.exe /GetStatus| |
| Shutdown| Cierra el Emulador de Azure Cosmos DB.| CosmosDB.Emulator.exe /Shutdown | |
...
| PartitionCount | Especifica el número máximo de las colecciones particionadas. Consulte [Cambio del número de colecciones](#set-partitioncount) para más información. | CosmosDB.Emulator.exe /PartitionCount=\<partitioncount\> | \<partitioncount\>: número máximo de colecciones de una sola partición permitidas. El valor predeterminado es 25. El máximo permitido es 250.|
| DefaultPartitionCount| Especifica el número predeterminado de particiones para una colección con particiones. | CosmosDB.Emulator.exe /DefaultPartitionCount=\<defaultpartitioncount\> | El valor predeterminado de \<defaultpartitioncount\> es 25.|
...
| NoFirewall | No ajuste las reglas de firewall cuando se utiliza /AllowNetworkAccess. |CosmosDB.Emulator.exe /NoFirewall | |
...
Con el emulador de Azure Cosmos DB, puede crear de forma predeterminada hasta 25 colecciones en una partición única o una sola colección con particiones. Mediante la modificación del valor **PartitionCount** puede crear hasta 250 colecciones de una sola partición o 10 colecciones con particiones, o cualquier combinación de ambas que no superen las 250 colecciones de una sola partición (una colección con particiones = 25 colecciones de una sola partición).
...
Use las siguientes sugerencias para solucionar los problemas que podrían surgir con el emulador de Azure Cosmos DB:
...
2. `CosmosDB.Emulator.exe /shutdown`. Vea la bandeja del sistema para asegurarse de que el programa se ha cerrado; esto podría tardar un minuto. También puede hacer clic en **Salir** en la interfaz de usuario del emulador de Azure Cosmos DB.
...
1. Cierre todas las instancias abiertas del emulador local; para ello, haga clic con el botón derecho en el icono del emulador de Azure Cosmos DB en la bandeja del sistema y luego haga clic en Salir. Todas las instancias pueden tardar un minuto en salir.
...
4. Cuando la aplicación se haya desinstalado, vaya a `C:\Users\<user>\AppData\Local\CosmosDBEmulator` y elimine la carpeta.
