...
* La **programación** incluye el uso de características básicas, como las cuotas de recursos y los presupuestos de interrupciones del pod. Para obtener más información acerca de estas características, consulte [Best practices for basic scheduler features in AKS][aks-best-practices-scheduler] (Procedimientos recomendados para las características básicas del programador en AKS).
  * Las características más avanzadas del programador incluyen manchas y tolerancias, selectores de nodo y afinidad o falta de afinidad del nodo y el pod. Para obtener más información acerca de estas características, consulte [Best practices for advanced scheduler features in AKS][aks-best-practices-advanced-scheduler] (Procedimientos recomendados para las características avanzadas del programador en AKS).
* Las **redes** incluyen el uso de directivas de red para controlar el flujo de tráfico de entrada y salida de pods.
* La **autenticación y autorización** incluyen el uso del control de acceso basado en rol (RBAC) y la integración de Azure Active Directory (AD), las identidades de pods y los secretos de Azure Key Vault. Para obtener más información acerca de estas características, consulte [Best practices for authentication and authorization in AKS][aks-best-practices-identity]. (Procedimientos recomendados para la autenticación y autorización en AKS).
...
Con el aislamiento lógico, un único clúster de AKS puede utilizarse para varios entornos, equipos o cargas de trabajo. Los [espacios de nombres][k8s-namespaces] de Kubernetes forman el límite de aislamiento lógico de cargas de trabajo y recursos.
