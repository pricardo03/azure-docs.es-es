...
- [Azure SQL Database](https://azure.microsoft.com/services/sql-database/): Un motor de SQL Database totalmente administrado, basado en la última edición Enterprise estable de SQL Server. Se trata de una base de datos relacional como servicio (DBaaS), hospedada en la nube de Azure, que se engloba en la categoría del sector denominada *Plataforma como servicio (PaaS)*. [Base de datos SQL](sql-database-technical-overview.md) se compila en hardware y software estandarizados que Microsoft posee, hospeda y mantiene. Con SQL Database, puede usar la funcionalidad y las características integradas que requieren configuración extensiva en SQL Server. Al utilizar SQL Database, se emplea el método de pago por uso de opciones para escalar vertical u horizontalmente a fin de aumentar la potencia de forma ininterrumpida. SQL Database incluye características adicionales que no están disponibles en SQL Server, como la administración y la inteligencia integradas. Azure SQL Database ofrece varias opciones de implementación:
  - Puede implementar una base de datos única a un [servidor lógico](sql-database-logical-servers.md). Un servidor lógico que contiene bases de datos únicas y agrupadas ofrece la mayoría de las características de ámbito de base de datos de SQL Server. Esta opción está optimizada para el desarrollo de las nuevas aplicaciones modernas procedentes de la nube.
...
| SQL Server en máquinas virtuales | Azure SQL Database (instancia administrada) | Azure SQL Database (servidores lógicos) |
| --- | --- | --- |
...
|  | Equipos que requieren mecanismos integrados de alta disponibilidad, recuperación ante desastres y actualización. | Igual que las bases de datos en un servidor lógico de Azure SQL Database. | Equipos que puedan configurar, ajustar, personalizar y administrar una alta disponibilidad, la recuperación ante desastres y la aplicación de revisiones para SQL Server. Algunas funciones automatizadas ya incorporadas simplifican considerablemente estos aspectos. | |
|  | Equipos que no desean administrar el sistema operativo subyacente ni los valores de configuración. | Igual que las bases de datos en un servidor lógico de Azure SQL Database. | Necesita un entorno personalizado con derechos administrativos completos. | |
|  | Bases de datos de hasta 100 TB. | Igual que las bases de datos en un servidor lógico de Azure SQL Database. | Instancias de SQL Server con hasta 64 TB de almacenamiento. La instancia puede admitir tantas bases de datos como sea necesario. |
...
| **Recursos:** | No desea emplear recursos de TI para la configuración y administración de la infraestructura subyacente, sino centrarse en el nivel de aplicación. | Igual que las bases de datos en un servidor lógico de Azure SQL Database. | Tiene algunos recursos de TI para la configuración y administración. Algunas funciones automatizadas ya incorporadas simplifican considerablemente estos aspectos. |
| **Costo total de propiedad:** | Elimina los costos de hardware y reduce los costos administrativos. | Igual que las bases de datos en un servidor lógico de Azure SQL Database. | Elimina costes de hardware. |
| **Continuidad del negocio:** |Además de las [funcionalidades integradas de infraestructura de tolerancia a errores](sql-database-high-availability.md), Azure SQL Database proporciona características que aumentan la continuidad empresarial, como, por ejemplo, [copias de seguridad automatizadas](sql-database-automated-backups.md), [restauración a un momento dado](sql-database-recovery-using-backups.md#point-in-time-restore), [restauración geográfica](sql-database-recovery-using-backups.md#geo-restore), [replicación geográfica activa](sql-database-active-geo-replication.md) y [grupos de conmutación por error](sql-database-auto-failover-group.md). Para más información, consulte [Información general: continuidad del negocio en la nube y recuperación ante desastres con la SQL Database](sql-database-business-continuity.md). | Igual que las bases de datos en un servidor lógico de Azure SQL Database; además, están disponibles las copias de seguridad que inició el usuario y de solo copia. | SQL Server en máquinas virtuales de Azure permite configurar una solución de recuperación ante desastres y alta disponibilidad para las necesidades específicas de su base de datos. Por consiguiente, podrá tener un sistema altamente optimizado para la aplicación. Podrá probar y ejecutar conmutaciones por error cuando sea necesario. Para más información, consulte [Alta disponibilidad y recuperación ante desastres para SQL Server en Azure Virtual Machines](../virtual-machines/windows/sql/virtual-machines-windows-sql-high-availability-dr.md). |
