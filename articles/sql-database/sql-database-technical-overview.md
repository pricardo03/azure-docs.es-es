...
- [Modelo sin servidor](sql-database-serverless.md) que escala automáticamente el proceso en función de la demanda de la carga de trabajo y se factura según la cantidad de proceso utilizado por segundo. El nivel de proceso sin servidor también detiene automáticamente las bases de datos durante períodos inactivos cuando solo se factura el almacenamiento y reanuda automáticamente las bases de datos cuando finaliza la actividad.
...
- Nivel de servicio [De uso general/estándar](sql-database-service-tier-general-purpose.md) diseñado para las cargas de trabajo comunes. Ofrece opciones de proceso y almacenamiento equilibradas adecuadas para un presupuesto limitado.
- Nivel de servicio [Crítico para la empresa/prémium](sql-database-service-tier-business-critical.md) diseñado para las aplicaciones de OLTP con un alto índice de transacciones y una latencia de E/S más baja. Ofrece la máxima resistencia a errores mediante varias réplicas aisladas.
- Nivel de servicio [Hiperescala](sql-database-service-tier-hyperscale.md) diseñado para bases de datos OLTP de gran tamaño y la capacidad para escalar el almacenamiento automáticamente y el proceso de manera fluida. 
