...
> [!IMPORTANT]
> Los productos de Azure CDN proporcionan todos la misma funcionalidad de filtrado geográfico, pero difieren ligeramente en los códigos de país que admiten. Para obtener más información, consulte [Azure CDN Country Codes](https://msdn.microsoft.com/library/mt761717.aspx) (Códigos de países de Azure CDN).
...
* Los cambios en la configuración de filtrado de país no surten efecto inmediatamente:
   ...
   * En los perfiles de **Azure CDN estándar de Verizon** y **Azure CDN premium de Verizon**, la propagación se completa normalmente en 10 minutos. 
...
* Se puede aplicar solo una regla a la misma ruta de acceso relativa. Es decir, no se pueden crear varios filtros de países que apunten a la misma ruta de acceso relativa. Sin embargo, una carpeta puede tener varios filtros de países, debido a la naturaleza recursiva de los mismos. En otras palabras, se puede asignar un filtro de país diferente a una subcarpeta de una carpeta configurada previamente.
