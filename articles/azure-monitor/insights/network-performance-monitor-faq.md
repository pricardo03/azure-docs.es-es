...
description: Este artículo incluyen las preguntas más frecuentes acerca de Network Performance Monitor en Azure. Network Performance Monitor (NPM) le ayuda a supervisar el rendimiento de las redes en tiempo real y detectar y localizar cuellos de botella de rendimiento de red.
...
- Del NPM Monitor de rendimiento y las capacidades del Monitor de conectividad de servicios compatible con el servidor de Windows y sistemas operativos de equipos de escritorio/cliente de Windows. Versiones de sistema operativo de servidor de Windows admitidas son 2008 SP1 o posterior. Las versiones de equipos de escritorio/cliente de Windows compatibles son Windows 10, Windows 8.1, Windows 8 y Windows 7. 
...
La capacidad de supervisar las redes mediante los nodos basados en Linux está actualmente en versión preliminar. Póngase en contacto con el administrador de cuentas para obtener más información. Los agentes de Linux solo ofrecen funcionalidad de supervisión con la funcionalidad Monitor de rendimiento de NPM y no está disponible para las funcionalidades Monitor de conectividad de servicio y Supervisión de ExpressRoute.
...
El Monitor de rendimiento y las capacidades del Monitor de conectividad de servicio admiten nodos [conectado como agentes directos](../../azure-monitor/platform/agent-windows.md) y [conectados a través de Operations Manager](../../azure-monitor/platform/om-agents.md).

En la funcionalidad Supervisión de ExpressRoute, los nodos de Azure solo deben conectarse como agentes directos. Los nodos de Azure, que se conectan a través de Operations Manager, no se admiten. Para los nodos locales, se admiten los nodos conectados como los agentes directos y a través de Operations Manager para supervisar un circuito ExpressRoute.
...
Si está supervisando la red mediante los nodos basados en servidor en Windows, se recomienda que usar TCP como protocolo de supervisión, ya que proporciona una mayor precisión. 

Se recomienda ICMP para los nodos basados en el sistema operativo de cliente o escritorios de Windows. Este does'nt plataforma permitir datos TCP se envíen a través de sockets sin formato, que usa NPM para detectar la topología de red.
...
### <a name="what-is-the-maximum-number-of-agents-i-can-use-or-i-see-error--youve-reached-your-configuration-limit"></a>¿Qué es el número máximo de agentes que puedo usar o veo el error de "... ¿ha alcanzado el límite de configuración"?
NPM limita el número de direcciones IP a direcciones 5000 por área de trabajo. Si un nodo tiene direcciones IPv4 e IPv6, esto contará como dos direcciones IP para ese nodo. Por lo tanto, el límite de 5000 direcciones IP podría decidir el límite superior en el número de agentes. Puede eliminar los agentes inactivos en la pestaña Nodos en NPM >> Configurar. NPM también mantiene el historial de todas las direcciones IP que nunca se asignaron a la máquina virtual hospeda el agente y cada uno se consideran que contribuyen a ese límite de 5000 las direcciones IP IP independiente. Para liberar direcciones IP para el área de trabajo, puede usar la página de nodos para eliminar las direcciones IP que no están en uso.
...
### <a name="why-does-my-link-show-unhealthy-but-the-topology-does-not"></a>¿Por qué mi show vínculo incorrecto pero la topología no 
NPM supervisa la topología en intervalos diferentes, latencia y pérdida de extremo a otro. Pérdida y latencia se mide una vez cada 5 segundos y se agregan a cada tres minutos (Monitor de rendimiento y Monitor de ruta Express) mientras la topología se calcula utilizando traceroute cada 10 minutos. Por ejemplo, entre 44:3 y 4:04, topología puede actualizarse tres veces (3:44, 54:3, 4:04), pero se actualizan pérdida y latencia unas siete veces (3:44, 3:47, 3:50, 3:53, 56:3 de 3:59, 4:02). La topología que se generan en 3:54 se representa por la pérdida y latencia que obtiene calculada en 56:3 de 3:59 y 4:02. Supongamos que reciba una alerta que el circuito ER estaba en mal estado a las 3:59. Inicie sesión en NPM e intente establecer la hora de la topología hasta las 3:59. NPM representará la topología que se generan en 3:54. Para entender la topología último conocida de la red, compare los campos TimeProcessed (tiempo en que se calculó la pérdida y latencia) y TracerouteCompletedTime (tiempo en que se calculó la topología). 
...
E2EMedianLatency es la latencia que se actualiza cada tres minutos después de agregar los resultados de pruebas de ping de tcp, mientras que es AvgHopLatencyList actualizado cada 10 minutos en función de traceroute. Para comprender la hora exacta a la que se calculó E2EMedianLatency, utilice el campo TimeProcessed. Para entender la hora exacta en qué traceroute completado y AvgHopLatencyList actualizada, use el campo TracerouteCompletedTime

### <a name="why-does-hop-by-hop-latency-numbers-differ-from-hoplatencyvalues"></a>¿Por qué se diferencian las cifras de latencia de salto a salto del HopLatencyValues 
HopLatencyValues son el origen al punto de conexión.
Por ejemplo: Saltos - A, B, C. AvgHopLatency - 10,15,20. Esto significa que el origen para una latencia = 10, origen a la latencia de B = 15 y origen a la latencia de C es 20. Interfaz de usuario calculará la latencia de salto A y B como 5 de la topología
