...
Requiere una consulta de registros para recuperar datos de Log Analytics.  Si está [analizando los datos en el portal](../../azure-monitor/log-query/portals.md), [configurando una regla de alertas](../../azure-monitor/platform/alerts-metric.md) para recibir una notificación sobre una condición determinada o recuperando datos mediante la [API de Log Analytics](https://dev.loganalytics.io/), usará una consulta para especificar los datos que quiere.  En este artículo se describe cómo se usan las búsquedas de registros en Log Analytics y se proporcionan los conceptos que debe comprender antes de crear una.
...
- **Portales.** Puede realizar análisis interactivos de los datos de registro en [Azure Portal](../../azure-monitor/log-query/portals.md).  Esto le permite modificar la consulta y analizar los resultados en una gran variedad de formatos y visualizaciones.  
- **Reglas de alertas.** [Las reglas de alertas](../../azure-monitor/platform/alerts-overview.md) identifican de manera proactiva los problemas de datos del área de trabajo.  Cada regla de alertas se basa en una búsqueda de registros que se ejecuta automáticamente a intervalos regulares.  Los resultados se inspeccionan para determinar si se debe crear una alerta.
...
- **Exportación.**  Cuando importe datos desde el área de trabajo de Log Analytics a Excel o [Power BI](../../azure-monitor/platform/powerbi.md), cree una consulta de registros para definir los datos que se van a exportar.
...
Cuando compila una consulta, primero debe determinar las tablas que tienen los datos que está buscando. Los diferentes tipos de datos se dividen en tablas dedicadas en cada [área de trabajo de Log Analytics](../../azure-monitor/learn/quick-create-workspace.md).  La documentación de los distintos orígenes de datos incluye el nombre del tipo de datos que crea y una descripción de cada una de sus propiedades.  Muchas consultas solo necesitan datos de una sola tabla, pero otras pueden utilizar una variedad de opciones para incluir datos de varias tablas.

Mientras que [Application Insights](../../azure-monitor/app/app-insights-overview.md) almacena los datos de la aplicación, como las solicitudes, las excepciones, los seguimientos y el uso de Log Analytics, estos datos se almacenan en una partición distinta de los otros datos del registro. Debe usar el mismo lenguaje de consulta para obtener acceso a estos datos, pero debe utilizar la [consola de Application Insights](../../azure-monitor/app/analytics.md) o la [API REST de Application Insights](https://dev.applicationinsights.io/) para acceder a ellos. Puede usar [consultas entre recursos](../../azure-monitor/log-query/cross-workspace-query.md) para combinar datos de Application Insights con otros datos de Log Analytics.
...
- Obtenga información acerca de los [portales que puede utilizar para crear y editar búsquedas de registros](../../azure-monitor/log-query/portals.md).
- Consulte un [tutorial sobre cómo escribir consultas](../../azure-monitor/log-query/get-started-queries.md) mediante el nuevo lenguaje de consulta.
