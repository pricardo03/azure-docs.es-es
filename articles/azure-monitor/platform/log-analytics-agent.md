...
Antes de analizar y actuar sobre los datos recopilados, primero deberá instalar y conectar a agentes en todas las máquinas que desea enviar datos al servicio Azure Monitor. Puede instalar agentes en las máquinas virtuales de Azure mediante la extensión de máquinas virtuales de Azure Log Analytics para Windows y Linux, y para las máquinas de un entorno híbrido mediante configuración, la línea de comandos o con Desired State Configuration (DSC) en Azure Automation. 

El agente para Linux y Windows se comunica con el servicio Azure Monitor a través del puerto TCP 443 y, si el equipo se conecta a través de un servidor proxy o firewall para comunicarse a través de Internet, revise los requisitos siguientes para comprender la configuración de red Obligatorio. Si las directivas de seguridad de TI no permitir que los equipos de la red para conectarse a Internet, puede configurar un [puerta de enlace de Log Analytics](gateway.md) y, a continuación, configure el agente pueda conectarse a través de la puerta de enlace a los registros de Azure Monitor. El agente, a continuación, puede recibir información de configuración y enviar los datos recopilan según los datos que las reglas de recopilación y supervisión de las soluciones se ha habilitado en el área de trabajo. 

Si va a supervisar un equipo con System Center Operations Manager 2012 R2 o posterior, puede ser múltiple con el servicio de Azure Monitor para recopilar datos y reenviarlos al servicio y ser supervisados por [Operations Manager](../../azure-monitor/platform/om-agents.md). Con los equipos Linux, el agente no incluye un componente de servicio de mantenimiento que el agente de Windows, e información se recopila y procesa un servidor de administración en su nombre. Dado que los equipos de Linux se supervisan de manera diferente con Operations Manager, no recibe la configuración o recopilar datos directamente y reenviar a través del grupo de administración como un sistema de Windows administrados con agente. Como resultado, este escenario no es compatible con equipos Linux informan a Operations Manager.  
...
La información siguiente muestra la información de configuración de proxy y firewall requerida para que el agente de Linux y Windows para comunicarse con los registros de Azure Monitor.  
...
El agente de Windows y Linux admite la comunicación a través de un servidor proxy o puerta de enlace de Log Analytics a Azure Monitor mediante el protocolo HTTPS.  Se admite la autenticación anónima y básica (nombre de usuario/contraseña).  Si el agente de Windows está conectado directamente al servicio, la configuración de proxy se especifica durante la instalación o [después de la implementación](agent-manage.md#update-proxy-settings) desde el Panel de control o con PowerShell.  
...
Conectar los equipos en su suscripción de Azure o un entorno híbrido directamente con los registros de Azure Monitor puede realizarse mediante métodos diferentes según sus requisitos. En la tabla siguiente se resalta cada método para determinar cuál funciona mejor en su organización.
...
| System Center Operations Manager|[Integrar Operations Manager con Log Analytics](../../azure-monitor/platform/om-agents.md) | Configurar la integración entre los registros de Operations Manager y Azure Monitor para reenviar datos procedentes de equipos de Windows que informan a un grupo de administración.|  
