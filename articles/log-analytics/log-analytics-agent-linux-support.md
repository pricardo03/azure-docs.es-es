...
description: Aquí se describen los síntomas, las causas y las soluciones de los problemas más comunes que surgen con el agente de Log Analytics para Linux.
...
En este artículo se proporciona información acerca de los errores que puede experimentar con el agente de Log Analytics para Linux y se sugieren posibles soluciones para resolverlos.
...
* Visite la página de comentarios de Log Analytics para revisar los errores e ideas enviadas [http://aka.ms/opinsightsfeedback](https://aka.ms/opinsightsfeedback) o registre uno nuevo.  
...
## <a name="important-configuration-files"></a>Archivos de configuración importantes
 ...
 >[!NOTE]
 >Edición de archivos de configuración para los contadores de rendimiento, y Syslog se sobrescribe si la colección se configura desde el [menú de datos Configuración avanzada de Log Analytics](log-analytics-data-sources.md#configuring-data-sources) en Azure Portal para el área de trabajo. Para deshabilitar la configuración para todos los agentes, deshabilite la colección de **Configuración avanzada** de Log Analytics o, para un único agente, ejecute lo siguiente:  
...
| 4 | Tipo de paquete no válido o configuración de proxy no válida; los paquetes omsagent-*rpm*.sh solo pueden instalarse en sistemas basados en RPM, y los paquetes omsagent-*deb*.sh solo pueden instalarse en sistemas basados en Debian. Se recomienda usar el instalador universal de la [versión más reciente](log-analytics-quick-collect-linux-computer.md#install-the-agent-for-linux). Además, [revise](#issue:-unable-to-connect-through-proxy-to-log-analytics) para comprobar la configuración de proxy. |
...
| 4 | Proxy no válido proporcionado al script omsadmin. Compruebe el proxy y consulte nuestra [documentación para usar un proxy HTTP](log-analytics-agent-overview.md#network-firewall-requirements). |
| 5 | Se ha recibido el error HTTP 403 del servicio de Log Analytics. Vea el resultado completo del script omsadmin para obtener más información. |
| 6 | Se ha recibido el error HTTP distinto de 200 del servicio de Log Analytics. Vea el resultado completo del script omsadmin para obtener más información. |
| 7 | No se puede conectar al servicio de Log Analytics. Vea el resultado completo del script omsadmin para obtener más información. |
...
El registro de depuración permite ver cargas por lotes al servicio de Log Analytics separadas por tipo, número de elementos de datos y tiempo de envío:
...
### <a name="resolution"></a>Resolución
* Compruebe que la configuración de Syslog en el área de trabajo de Log Analytics tenga todas las utilidades y los niveles de registro correctos. Revise [Configuración de Syslog en Azure Portal](log-analytics-data-sources-syslog.md#configure-syslog-in-the-azure-portal).
...
### <a name="probable-causes"></a>Causas probables
* Se produjo un error en la incorporación al servicio de Log Analytics
* Se bloqueó la conexión al servicio de Log Analytics
...
### <a name="resolution"></a>Resolución
**Información previa:** `omsconfig` es el agente de configuración del agente de Log Analytics para Linux que se busca la nueva configuración del portal cada cinco minutos. Esta configuración se aplica luego en los archivos de configuración del agente de Log Analytics para Linux ubicados en /etc/opt/microsoft/omsagent/conf/omsagent.conf.
...
2. Instale el agente siguiendo las [instrucciones](log-analytics-quick-collect-linux-computer.md).
