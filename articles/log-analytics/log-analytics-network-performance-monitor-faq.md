...
description: En este artículo incluyen las preguntas más frecuentes sobre NPM en Azure. Network Performance Monitor (NPM) ayuda a supervisar el rendimiento de sus redes (es tiempo casi real) para detectar y localizar los cuellos de botella de rendimiento que afectan a la red.
...
- Las funcionalidades Monitor de conectividad de servicio y Supervisión del rendimiento de NPM admiten tanto Windows Server (2008 SP1 o posterior) como escritorios/sistemas operativos cliente Windows (Windows 10, Windows 8.1, Windows 8 y Windows 7). 
...
Las dos funcionalidades, Supervisión del rendimiento y Monitor de conectividad de servicio, admiten nodos [conectados como agentes directos](log-analytics-agent-windows.md), así como [conectados a través de Operations Manager](log-analytics-om-agents.md).

En la funcionalidad Supervisión de ExpressRoute, los nodos de Azure solo deben conectarse como agentes directos. Los nodos de Azure, que se conectan a través de Operations Manager, no se admiten. En los nodos locales, los nodos conectados como agentes directos y a través de Operations Manager se admiten para supervisar un circuito ExpressRoute.
...
Para los nodos basados en escritorios/sistemas operativos cliente Windows, se recomienda utilizar ICMP, ya que esta plataforma no permite el envío de datos TCP a través de sockets sin procesar, lo que es necesario para que NPM detecte la topología de la red.

Puede obtener más detalles sobre las ventajas relativas de cada protocolo [aquí](log-analytics-network-performance-monitor-performance-monitor.md#choose-the-protocol).
...
* Ejecute el script [EnableRules.ps1](https://aka.ms/npmpowershellscript) de Powershell en el nodo. Vea las [instrucciones](log-analytics-network-performance-monitor.md#configure-log-analytics-agents-for-monitoring) para obtener más detalles.
...
NPM puede supervisar la conectividad entre redes en cualquier parte del mundo, desde un área de trabajo que se hospede en una de las [regiones admitidas](log-analytics-network-performance-monitor.md#supported-regions).
...
NPM puede supervisar la conectividad con servicios en cualquier parte del mundo, desde un área de trabajo que se hospede en una de las [regiones admitidas](log-analytics-network-performance-monitor.md#supported-regions).
...
Esto puede suceder si el circuito y las conexiones de emparejamiento se distribuyen entre varias suscripciones. NPM solo detecta las conexiones de emparejamiento privado de ExpressRoute en las que las redes virtuales conectadas a ExpressRoute se encuentran en la misma suscripción que la vinculada al área de trabajo de NPM. Además, NPM detecta las conexiones de emparejamiento de Microsoft en las que el circuito ExpressRoute conectado se encuentra en la misma suscripción que la vinculada al área de trabajo de NPM. Esto se puede aclarar en el siguiente ejemplo:

 Si tiene dos redes virtuales, VNET A en la suscripción A y VNET B en la suscripción B respectivamente, conectadas a ExpressRoute en la suscripción C. Además, hay otra red virtual, VNET C en la suscripción C. La red ER también tiene emparejamiento MS en la suscripción de C. 

A continuación,

* Si el área de trabajo NPM está vinculada a la suscripción A, solo podrá supervisar la conectividad a través de ER con VNET A.
* Si el área de trabajo NPM está vinculada a la suscripción B, solo podrá supervisar la conectividad a través de ER con VNET B.
* Si el área de trabajo NPM está vinculada a la suscripción C, podrá supervisar la conectividad a través de ER con VNET C además del emparejamiento MS.

La compatibilidad entre suscripciones pronto estará disponible. Después de esto, podrá supervisar todas sus conexiones privadas de ExpressRoute y de emparejamiento de Microsoft en diferentes suscripciones, desde una sola área de trabajo.
...
- Puede obtener más información sobre Network Performance Monitor en [Solución Network Performance Monitor en Azure](log-analytics-network-performance-monitor.md).
