...
* Una solución de integración continua Hudson.
  ...
  5. Después de terminar la configuración inicial, cancele la instancia en ejecución de Hudson WAR, vuelva a iniciar Hudson WAR y vuelva a abrir el panel de Hudson, `http://localhost:8080/`, que usará para instalar y configurar el complemento de Azure Storage.
...
3. En la sección **Microsoft Azure Storage Account Configuration** (Configuración de la cuenta de almacenamiento de Microsoft Azure):
    ...
    c. Use el valor predeterminado para **Blob Service Endpoint URL** (Dirección URL del extremo de servicio BLOB) si usa la nube pública de Azure. Si usa una nube de Azure distinta, use el punto de conexión tal y como se especifica en [Azure Portal](https://portal.azure.com) para la cuenta de almacenamiento.
...
* **Formato de dirección URL**: Los blobs son direccionables con el siguiente formato de dirección URL:
    ...
    (El formato anterior se aplica a la nube pública de Azure. Si usa una nube de Azure distinta, use el punto de conexión en el [Azure Portal](https://portal.azure.com) para determinar el punto de conexión de la URL).
