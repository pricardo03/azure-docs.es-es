...
1. Seleccione el botón **Agregar métrica** para ver varias métricas trazadas en el mismo gráfico.
1. Seleccione el botón **+ Nuevo gráfico** para ver varios gráficos en una vista.
...
**Métrica** | **Unidad** | **Agregación** | **Descripción de la métrica**
|---|---|---|---|
Continuous export number of exported records (Exportación continua: número de registros exportados)    | Count | Sum | Total de registros exportados desde el clúster. |
Continuous export max lateness minutes (Máx. de minutos de demora en la exportación continua) |    Count   | Max   | Valor máximo en minutos de los registros exportados.|
Continuous export pending count (Recuento pendiente de exportaciones continuas) | Count | Max   | Valor máximo de operaciones de exportación pendientes.
Continuous export result (Resultado de la exportación continua)    | Count |   Count   | Total de operaciones de exportación continuas, por resultado. La métrica incluye un nombre de exportación continua y una base de datos. 
Utilización de la exportación |    Percent | Max   | Uso de la ranura definida para las operaciones de exportación.
| | | | |
...
**Métrica** | **Unidad** | **Agregación** | **Descripción de la métrica**
|---|---|---|---|
| Events processed (for Event/IoT Hubs) [Eventos procesados (para instancias de Event Hubs o IoT Hub)] | Count | Máximo, mínimo, suma | Número total de eventos leídos de los centros de eventos y procesados por el clúster. Los eventos se dividen en eventos rechazados y aceptados por el motor del clúster. |
...
| Volumen de ingesta (en MB) | Count | Máxima, suma | El tamaño total de datos ingeridos en el clúster (en MB) antes de la compresión. |
| | | | |  
...
**Métrica** | **Unidad** | **Agregación** | **Descripción de la métrica**
|---|---|---|---|
| Duración de la consulta | Milisegundos | Media, mínima, máxima, suma | Tiempo total hasta que se reciben los resultados de la consulta (no incluye la latencia de red). |
...
**Métrica** | **Unidad** | **Agregación** | **Descripción de la métrica**
|---|---|---|---|
Streaming Ingest Data Rate (Velocidad de datos de la ingesta de streaming) |    Count   | RateRequestsPerSecond | Volumen total de datos ingeridos en el clúster. |
Streaming Ingest Duration (Duración de la ingesta de streaming)   | Milisegundos  | Media, máxima, mínima | Duración total de las solicitudes de ingesta de streaming. |
Streaming Ingest Request Rate [Velocidad de solicitudes de ingesta de streaming]   | Count | Recuento, media, máxima, mínima, suma | Total de solicitudes de ingesta de streaming. |
Streaming Ingest Result (Resultado de la ingesta de streaming) | Count | Avg   | Total de solicitudes de ingesta de streaming por tipo de resultado. |
| | | | |
