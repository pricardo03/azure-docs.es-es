...
### <a name="where-can-i-track-feature-updates-from-now-on"></a>¿Dónde puedo comprobar actualizaciones de características en el futuro?
De ahora en adelante, se publicarán las actualizaciones de características y entradas de blog informativo en el blog de Azure y las actualizaciones de Azure. Estas entradas de blog también se vincularán a la documentación de cada vez que necesite.
...
Usa una cuenta de Microsoft para casi todo lo que hace con los servicios y los dispositivos de Microsoft. Es una dirección de correo electrónico y contraseña que usó para iniciar sesión en Skype, Outlook.com, OneDrive, Windows phone, Azure y Xbox Live. Tener una sola cuenta significa que puede disponer de los archivos, fotografías, contactos y configuración desde cualquier dispositivo.
 
> [!NOTE]
> Una cuenta de Microsoft anteriormente se conocía como un Windows Live ID.
...
Autoservicio sin preocupaciones significa que los desarrolladores y evaluadores crean sus propios entornos según sea necesario. Los administradores tienen la seguridad de saber que DevTest Labs puede ayudar a establecer el acceso adecuado, minimice el derroche y controlar los costos. Los administradores pueden especificar qué tamaños de máquina virtual se permiten, el número máximo de máquinas virtuales y cuándo iniciar y apagar las máquinas virtuales. DevTest Labs también permite supervisar fácilmente los costos y definir alertas para ayudarle a estar al tanto de cómo se usan los recursos de laboratorios.
...
Seguridad del acceso viene determinada por el Control de acceso basado en roles (RBAC). Para aprender cómo funciona el acceso, es importante comprender las diferencias entre un permiso, un rol y un ámbito según se define en RBAC.
...
- **Usuario de laboratorio**: puede ver todos los recursos del laboratorio, como las máquinas virtuales, las directivas y las redes virtuales. Sin embargo, un usuario de laboratorio no puede modificar las directivas ni las máquinas virtuales creadas por otros usuarios.
...
Puesto que los ámbitos son jerárquicos, cuando un usuario tiene permisos en un ámbito determinado, también se le conceden automáticamente en cada ámbito de nivel inferior. Por ejemplo, si un usuario está asignado al rol de propietario de la suscripción, tiene acceso a todos los recursos de una suscripción. Estos recursos incluyen máquinas virtuales, redes virtuales y laboratorios. El propietario de una suscripción hereda automáticamente el rol de propietario de laboratorio. Sin embargo, lo contrario no es cierto. El propietario de un laboratorio tiene acceso a un laboratorio, que es un ámbito inferior al del nivel de suscripción. Por lo tanto, un propietario de laboratorio no puede ver las máquinas virtuales, redes virtuales o cualquier otro recurso que esté fuera del laboratorio.
...
Central TI debe poseer solo lo que es necesario y permitir que los equipos de proyecto y la aplicación para que el nivel necesario de control. Normalmente, esto significa que esa departamento de TI central posee la suscripción y las controla las funciones de TI básicas, como las configuraciones de redes. El conjunto de **propietarios** para una suscripción debe ser pequeño. Estos propietarios pueden designar a propietarios adicionales cuando es necesario, o aplicar directivas de nivel de suscripción, por ejemplo "sin dirección IP pública".
...
La primera capa de controles para el acceso remoto es aplicar una directiva de acceso remoto a través de una conexión VPN que no está conectada directamente al laboratorio.

La segunda capa de controles consiste en aplicar un conjunto de objetos de directiva de grupo que impiden copiar y pegar mediante el escritorio remoto. Se podría implementar una directiva de red para no permitir que los servicios salientes del entorno, como los servicios FTP y RDP, queden fuera del entorno. Enrutamiento definido por el usuario podría forzar todo el tráfico de red Azure a local, pero no se pudo el enrutamiento de la cuenta para todas las direcciones URL que podrían permitir la carga de datos a menos que se controla a través de un proxy para examinar el contenido y las sesiones. Las direcciones IP públicas podrían restringirse dentro de la red virtual que admite DevTest Labs para no permitir el protocolo de puente de un recurso de red externo.

En última instancia, debe aplicarse a través de la organización, que sería todos los métodos posibles de medios extraíbles o direcciones URL externas que podrían aceptar una publicación de contenido del mismo tipo de restricciones. Consulte con el profesional de seguridad para revisar e implementar una directiva de seguridad. Para más recomendaciones, consulte [Ciberseguridad de Microsoft](https://www.microsoft.com/security/default.aspx?&WT.srch=1&wt.mc_id=AID623240_SEM_sNYnsZDs).
...
Hemos ofrecido un [repositorio de GitHub de plantillas de Azure Resource Manager para laboratorios](https://azure.microsoft.com/resources/templates/101-dtl-create-lab) que se pueden implementar tal cual o modificar para crear plantillas personalizadas para los laboratorios. Cada plantilla tiene un vínculo a implementar el laboratorio porque se encuentra en su propia suscripción de Azure. O bien, puede personalizar la plantilla e [implementarla con PowerShell o la CLI de Azure](../azure-resource-manager/resource-group-template-deploy.md).


### <a name="can-i-have-all-virtual-machines-to-be-created-in-a-common-resource-group-instead-having-each-machine-in-its-own-resource-group"></a>¿Puedo tener todas las máquinas virtuales que se creará en un grupo de recursos comunes en su lugar, tener cada máquina en su propio grupo de recursos? 
Sí, como propietario del laboratorio, puede dejar que el laboratorio de controlar la asignación de grupo de recursos para usted o todas las máquinas virtuales creado en un grupo de recursos comunes que especifique. 

Escenario de grupo de recursos independiente:
-   DevTest Labs crea un nuevo grupo de recursos para cada máquina de virtual IP pública y privada que ponga en marcha
-   DevTest Labs crea un grupo de recursos para máquinas IP compartidas que pertenecen al mismo tamaño.

Escenario de grupo de recursos comunes:
-   Todas las máquinas virtuales se ponen en marcha en el grupo de recursos comunes que especifique. Obtenga más información [asignación de grupo de recursos para el laboratorio](https://aka.ms/RGControl). 
...
### <a name="how-do-i-determine-the-ratio-of-users-per-lab-and-the-overall-number-of-labs-that-are-needed-across-an-organization"></a>¿Cómo se puede determinar la proporción de usuarios por laboratorio y el número total de laboratorios que se necesitan en toda la organización?
...
También debe tener en cuenta los límites geográficos. Por ejemplo, los desarrolladores en el norte este de Estados Unidos (US) pueden usar un laboratorio aprovisionado en East US2. Además, los desarrolladores de Dallas (Texas) y Denver (Colorado) pueden ser remitidos a usar un recurso en Centro y Sur de EE. UU. Si existe algún trabajo de colaboración con un tercero externo, se les podría asignar a un laboratorio que no utilizan los desarrolladores internos.

También puede usar un laboratorio para un proyecto específico dentro de los proyectos de DevOps de Azure. A continuación, aplique seguridad en un grupo específico de Azure Active Directory, lo que permite acceder a ambos conjuntos de recursos. La red virtual asignada al laboratorio puede ser otro límite para consolidar a los usuarios.
...
Se recomienda establecer los permisos adecuados a nivel de laboratorio para que solo los usuarios autorizados puedan eliminar los recursos o cambiar las directivas del laboratorio. Los desarrolladores deben incorporarse al grupo de **usuarios de DevTest Labs**. El jefe de desarrollo o el responsable de la infraestructura debe ser el **propietario de DevTest Labs**. Se recomienda que tener sólo dos de los propietarios de laboratorio. Esta directiva se extiende al repositorio de código para evitar daños. Los usuarios del laboratorio tienen derechos para usar los recursos, pero no pueden actualizar las directivas de laboratorio. Consulte el artículo siguiente en el que se enumeran los roles y derechos que cada grupo integrado tiene dentro de un laboratorio: [Adición de propietarios y usuarios en Azure DevTest Labs](devtest-lab-add-devtest-user.md).

### <a name="how-do-i-share-a-direct-link-to-my-lab"></a>¿Cómo se puede compartir un vínculo directo a mi laboratorio?

1. En el [portal Azure](https://portal.azure.com), vaya al laboratorio.
2. Copia el **dirección URL del laboratorio** desde el explorador y, a continuación, compartirlo con los usuarios del laboratorio.

> [!NOTE]
> Si un usuario de laboratorio es un usuario externo que tiene una cuenta de Microsoft, pero que no es un miembro de instancia de Active Directory de su organización, el usuario podría ver un mensaje de error cuando intenten tener acceso al vínculo compartido. Si un usuario externo ve un mensaje de error, pídale que seleccione primero su nombre en la esquina superior derecha de Azure Portal. A continuación, en la sección de directorio del menú, el usuario puede seleccionar el directorio donde existe el laboratorio.
...
Cuando se crea una máquina virtual de DevTest Labs, se le asigna el permiso de acceso a esa máquina virtual. Puede ver la máquina virtual en la página de laboratorios y en el **máquinas virtuales** página. Los usuarios asignados a la **DevTest Labs propietario** rol puede ver todas las máquinas virtuales que se crearon en el laboratorio en el laboratorio **todas las máquinas virtuales** página. Sin embargo, los usuarios que tienen el **usuario de DevTest Labs** rol no se concede automáticamente acceso de lectura a los recursos de máquina virtual que han creado otros usuarios. Por lo tanto, esas máquinas virtuales no se muestran en el **máquinas virtuales** página.
...
- También puede [generar una plantilla de Resource Manager](devtest-lab-add-vm.md#save-azure-resource-manager-template) al crear una máquina virtual, y [implementar la plantilla de Resource Manager desde Windows PowerShell](../azure-resource-manager/resource-group-template-deploy.md).
- También puede especificar más de una instancia de una máquina que se creará durante la creación de máquinas virtuales. Para más información acerca de cómo crear varias instancias de máquinas virtuales, consulte la documentación sobre [crear una máquina virtual de laboratorio](devtest-lab-add-vm.md).
...
2.  Crear el [imagen personalizada](devtest-lab-create-template.md) dentro del laboratorio de DevTest Labs.
...
Para utilizar imágenes de sistema operativo de cliente de Windows (Windows 7 o una versión posterior) para el desarrollo o pruebas en Azure, siga uno de los pasos siguientes:
...
Como propietario del laboratorio, puede eliminar las máquinas virtuales del laboratorio en Azure portal. También puede eliminar todas las máquinas virtuales en el laboratorio mediante un script de PowerShell. En el ejemplo siguiente, en el **valores pueden cambiar** comentario, modifique los valores de parámetro. Puede recuperar el subscriptionId y labResourceGroup labName valores del panel del laboratorio en Azure portal.
...
Implementar las plantillas de Resource Manager en un entorno de DevTest Labs mediante el uso de los pasos mencionados en el [característica de entornos en DevTest Labs](devtest-lab-test-env.md) artículo. Básicamente, consulte las plantillas de Resource Manager en un repositorio de GIT (Azure Repos o GitHub) y agregue un [repositorio privado para las plantillas](devtest-lab-test-env.md) al laboratorio. Este escenario no puede ser útil si usa DevTest Labs para hospedar máquinas de desarrollo, pero puede resultar útil si va a crear un entorno de ensayo, que es representativa de producción.

También merece la pena tener en cuenta que el número de máquinas virtuales por laboratorio o por la opción de usuario solo limita el número de máquinas creadas de forma nativa en el laboratorio en Sí y no mediante los entornos (plantillas de Resource Manager).
...
Normalmente, los factores decisivos en este escenario son el costo y la reutilización. Si tiene un escenario donde muchos usuarios/labs requieren una imagen con una gran cantidad de software sobre la imagen base, podría reducir los costos mediante la creación de una imagen personalizada. Significa que una vez creada la imagen. Reduce el tiempo de configuración de la máquina virtual y el costo en que se incurre por la ejecución de la máquina virtual durante la configuración.
...
Para obtener una explicación más detallada, consulte [comparación de imágenes personalizadas y fórmulas](devtest-lab-comparing-vm-base-image-types.md) en DevTest Labs.
...
- Instalación y configuración de una aplicación podrían tardar muchas horas, que no son un uso eficaz de tiempo de proceso que se agregará una imagen de Marketplace de Azure.
...
- Uso de imágenes personalizadas no debe interpretarse como a la ligera. Introduce una complejidad adicional, como ahora debe administrar los archivos de disco duro virtual para los subyacentes de las imágenes base. También debe revisar periódicamente esas imágenes base con las actualizaciones de software. Estas actualizaciones incluyen nuevas actualizaciones del sistema operativo (SO) y las actualizaciones o los cambios de configuración necesarios para el paquete de software.
...
- Cree un grupo denominado `All DevTest Labs Developers` en Azure Active Directory que se administra centralmente. Cualquier desarrollador que contribuya al desarrollo de artefacto debe colocarse en este grupo.
...
Si usa Azure DevOps, puede usar un [extensión DevTest Labs Tasks](https://marketplace.visualstudio.com/items?itemName=ms-azuredevtestlabs.tasks) para automatizar la canalización de versión en DevTest Labs. Algunas de las tareas que puede realizar con esta extensión incluyen:
...
- Elimine la máquina virtual al final de la canalización de versiones cuando ya no es necesario.
...
- **El tráfico entrante** : si la máquina virtual no tiene una dirección IP pública, a continuación, que no puede acceder a internet. Un enfoque común consiste en garantizar que se establece una directiva de nivel de suscripción, de forma que ningún usuario puede crear una dirección IP pública.
- **El tráfico saliente** : si desea evitar que las máquinas virtuales acceso directo a internet pública y forzar el tráfico a través de un firewall corporativo, a continuación, puede enrutar el tráfico local a través de expressroute o VPN, mediante el uso de enrutamiento.

> [!NOTE]
> Si tiene un servidor proxy que bloquea el tráfico sin configuración de proxy, no olvide agregar excepciones a la cuenta de almacenamiento del artefacto del laboratorio.
...
Cuando un recurso es un elemento primario de otro recurso, el primario debe existir antes de crear el secundario. Si el recurso primario no existe, verá un **ParentResourceNotFound** mensaje. Si no se especifica una dependencia del recurso primario, es posible que el recurso secundario se implemente antes que el primario.

Las máquinas virtuales son recursos secundarios en un laboratorio en un grupo de recursos. Cuando se usan plantillas de Resource Manager para la implementación de máquinas virtuales con PowerShell, el nombre del grupo de recursos proporcionado en el script de PowerShell debe ser el del grupo de recursos del laboratorio. Para más información, vea, [Solución de errores comunes de implementación de Azure](../azure-resource-manager/resource-manager-common-deployment-errors.md).
...
Los errores de implementación de máquinas virtuales se capturan en los registros de actividad. Puede encontrar los registros de actividad de máquina virtual en **registros de auditoría** o **diagnósticos de máquina Virtual** en el menú de recursos en la página de máquinas virtuales del laboratorio (aparece la página después de seleccionar la máquina virtual desde la lista My virtual machines).
