...
|Este de EE. UU. 2     |     B1, B2, S0, S1, S2, S4, D1  <br>S8, S9   |    7<br>1    |
...
|Oeste de EE. UU.     |    B1, B2, S0, S1, S2, S4, D1  <br>S8, S9   |    7<br>2  |
|Oeste de EE. UU. 2    |    B1, B2, S0, S1, S2, S4, D1   <br>S8, S9  |    3<br>1     |
...
|Europa occidental     |    B1, B2, S0, S1, S2, S4, D1   <br>S8, S9    |    7<br>1     |
...
## <a name="supported-datasources"></a>Orígenes de datos compatibles
...
Azure Analysis Services proporciona seguridad a los datos confidenciales a varios niveles. En el nivel de servidor: firewall, autenticación de Azure, roles del administrador de servidor y cifrado del lado del servidor. Y en el nivel de modelo de datos, los roles de usuario, y la seguridad a nivel de fila y a nivel de objeto garantizan que los datos están seguros y los ver solo los usuarios que deben verlos.

### <a name="firewall"></a>Firewall

El firewall de Azure Analysis Services bloquea todas las conexiones de cliente, a excepción de las direcciones IP especificadas en las reglas. Configure reglas mediante la especificación de las direcciones IP permitidas por direcciones IP de cliente individuales o por intervalo. También se pueden permitir o bloquear las conexiones de Power BI (servicio). Configure el firewall y las reglas en el portal o mediante PowerShell. Para más información, consulte [Configure a server firewall](analysis-services-qs-firewall.md) (Configuración de un firewall del servidor).
...
Desarrolle e implemente modelos con [SQL Server Data Tools (SSDT) para Visual Studio](https://msdn.microsoft.com/library/mt204009.aspx) de forma gratuita. SSDT incluye plantillas de proyecto de Analysis Services que le permiten ponerse rápidamente a pleno funcionamiento. SSDT ya incluye la moderna funcionalidad de consultas de orígenes de datos y de mashup denominada Obtención de datos para los modelos tabulares 1400. Si está familiarizado con esta funcionalidad en Power BI Desktop y Excel 2016, ya sabe lo fácil que es crear consultas de orígenes de datos altamente personalizadas. Los proyectos de Microsoft Analysis Services también están disponibles para Visual Studio en forma de paquete VSIX instalable. [Descargar de Marketplace](https://marketplace.visualstudio.com/items?itemName=ProBITools.MicrosoftAnalysisServicesModelingProjects).
...
Azure Analysis Services se integra con las métricas de Azure, lo que proporciona un elevado número de métricas específicas de recursos para ayudarle a supervisar el rendimiento y el estado de los servidores. Para más información, vea [Supervisión de las métricas del servidor](analysis-services-monitor.md). Registre métricas con los [registros de diagnóstico de recursos de Azure](../monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs.md). Supervise y envíe registros a [Azure Storage](https://azure.microsoft.com/services/storage/), difúndalos a [Azure Event Hubs](https://azure.microsoft.com/services/event-hubs/) y expórtelos a [Log Analytics](https://azure.microsoft.com/services/log-analytics/), un servicio de [Azure](https://www.microsoft.com/cloud-platform/operations-management-suite). Para más información, consulte [Configuración del registro de diagnóstico](analysis-services-logging.md).
...
Dado que los modelos tabulares de Analysis Services de Azure son muy parecidos a los modelos tabulares de SQL Server Analysis Services, hay una amplia biblioteca de artículos compartidos de referencia sobre desarrollo, conceptos y procedimientos en la [documentación de SQL Server Analysis Services](https://docs.microsoft.com/sql/analysis-services/analysis-services). Los artículos de la documentación de SQL Server Analysis Services muestran si también se aplican a Azure Analysis Services con un banner APPLIES TO debajo del título.
...
Las cosas cambian rápidamente. Obtenga la información más reciente en el [blog del equipo de Analysis Services](https://blogs.msdn.microsoft.com/analysisservices/) y en el [blog de Azure](https://azure.microsoft.com/blog/).
