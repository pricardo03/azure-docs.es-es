...
- **Simplicidad de conectividad:** el uso de flujos de dispositivos elimina la necesidad de una configuración compleja de redes privadas virtuales para habilitar la conectividad con dispositivos de IoT.
...
- **Facilidad de uso en configuraciones de redes privadas:** para alcanzar un dispositivo, el servicio hace referencia a su id. de dispositivo en lugar de la dirección IP. Esto resulta útil cuando un dispositivo se encuentra dentro de una red privada y tiene una dirección IP privada, o bien cuando su dirección IP se asigna de manera dinámica y es desconocida para el lado del servicio.

## <a name="device-stream-workflows"></a>Flujos de trabajo de los flujos de dispositivos
Un flujo de dispositivos se inicia cuando el servicio solicita conectarse a un dispositivo al proporcionar su id. de dispositivo. Este flujo de trabajo en especial se adapta al patrón de comunicación de cliente/servidor, lo que incluye SSH y RDP donde un usuario intenta conectarse de manera remota al servidor SSH o RDP que se ejecuta en el dispositivo mediante un programa de cliente SSH o RDP.
...
5. El servicio recibe una notificación del resultado cuando el dispositivo acepta el flujo y pasa a crear su propio WebSocket al punto de conexión de streaming. De manera similar, recibe la información de autenticación y la dirección URL del punto de conexión de streaming desde IoT Hub.
...
Como se mencionó [anteriormente](#Overview), el dispositivo crea una conexión de salida al punto de conexión de streaming de IoT Hub durante el proceso de iniciación de los flujos de dispositivos. Los firewalls del dispositivo o de la red deben permitir conectividad de salida a la puerta de enlace de streaming a través del puerto 443 (se trata de una conexión WebSocket cifrada con TLS).
...
Incluimos dos ejemplos para mostrar el uso de los flujos de dispositivos por parte de las aplicaciones. El ejemplo *echo* muestra el uso programático de los flujos de dispositivos (mediante una llamada a las API de SDK). El ejemplo *local proxy* muestra el uso de la funcionalidad del SDK para tunelizar el tráfico de las aplicaciones estandarizadas (como SSH, RDP o web) a través de flujos de dispositivos.

### <a name="echo-sample"></a>Ejemplo Eco
El ejemplo Eco muestra el uso programático de flujos de dispositivos para enviar y recibir bytes entre la aplicación del servicio y la aplicación del dispositivo. Use los vínculos siguientes para acceder a las guías de inicio rápido (puede usar los programas del servicio y del dispositivo en lenguajes distintos, por ejemplo, el programa de dispositivo de C puede funcionar con el programa de servicio de C#):
...
| NodeJS | [Vínculo](quickstart-device-streams-echo-nodejs.md) | -                                                        |
...
En esta sección se describe el uso de los flujos de dispositivos para habilitar los escenarios SSH en un dispositivo a través de los flujos de dispositivos (el caso de RDP u otros protocolos cliente/servidor son similares mediante el uso del puerto correspondiente del protocolo).

El programa de instalación aprovecha los dos programas de *proxy local* que se muestran en la ilustración siguiente, es decir, *proxy local de dispositivo* y *proxy local de servicio*. Los servidores proxy locales son responsables de realizar el [protocolo de enlace de iniciación de flujos de dispositivos](#Device-stream-creation-flow) con IoT Hub y de interactuar con el cliente SSH y el demonio SSH mediante la programación de socket de cliente/servidor normal.
...
2. El dispositivo acepta la iniciación del flujo y se establece un túnel al punto de conexión de streaming de IoT Hub (como se explicó anteriormente).
...
4. El proxy local de servicio escucha en un puerto designado en espera de conexiones SSH nuevas desde el usuario (el puerto 2222 del ejemplo es arbitrario). El usuario apunta el cliente SSH al puerto del proxy local de servicio en el host local.

### <a name="notes"></a>Notas
- Los pasos anteriores se completan en un túnel de un extremo a otro entre el cliente SSH (a la derecha) y el demonio SSH (a la izquierda). 
...
- La opción de usar el puerto `2222` en el proxy local de servicio es una elección arbitraria. El proxy se puede configurar para que use cualquier otro puerto disponible.

- La elección del puerto `22` depende del protocolo y es específica para SSH en este caso. En el caso de RDP, se debe usar el puerto `3389`. Se puede configurar en los programas de ejemplo que se proporcionaron.

Use los vínculos siguientes para ver información sobre cómo ejecutar los programas de proxy local en el lenguaje que prefiera. Como en el ejemplo Eco, puede ejecutar los servidores proxy local de dispositivo y local de servicio en distintos lenguajes, porque son completamente interoperables.
