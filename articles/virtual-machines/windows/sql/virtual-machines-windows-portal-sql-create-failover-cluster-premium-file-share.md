...
ms.locfileid: "72757819"
---
# <a name="configure-sql-server-failover-cluster-instance-with-premium-file-share-on-azure-virtual-machines"></a>Configuración de una instancia de clúster de conmutación por error de SQL Server con un recurso compartido de archivos premium en Azure Virtual Machines
...
Una diferencia importante es que en un clúster de conmutación por error de VM de IaaS de Azure, se recomienda una sola NIC por servidor (nodo de clúster) y una sola subred. La red de Azure tiene redundancia física, que hace que las NIC y subredes adicionales sean innecesarias en un clúster invitado de VM de IaaS de Azure. Aunque el informe de validación del clúster emita una advertencia acerca de que los nodos solo son accesibles en una única red, esta advertencia puede omitirse en los clústeres de conmutación por error de VM de IaaS de Azure. 
...
> [!IMPORTANT]
> Actualmente, las instancias del clúster de conmutación por error de SQL Server de las máquinas virtuales de Azure solo se admiten con el modo de administración [lightweight](virtual-machines-windows-sql-register-with-resource-provider.md#register-with-sql-vm-resource-provider) de la [extensión del agente de IaaS de SQL Server](virtual-machines-windows-sql-server-agent-extension.md). Para cambiar del modo de extensión total a extensión ligera, elimine el recurso **Máquina virtual SQL** de las máquinas virtuales correspondientes y regístrelas con el proveedor de recursos de VM con SQL en el modo [ligero](virtual-machines-windows-sql-register-with-resource-provider.md#register-with-sql-vm-resource-provider). Al eliminar el recurso **Máquina virtual SQL** desde Azure Portal, desactive la casilla de la máquina virtual correcta. La extensión completa admite características como la copia de seguridad automatizada, y la aplicación de revisiones y la administración avanzada del portal. Estas características no funcionarán para las máquinas virtuales SQL después de que el agente se vuelva a instalar en modo de administración [ligera](virtual-machines-windows-sql-register-with-resource-provider.md#register-with-sql-vm-resource-provider).

### <a name="workload-consideration"></a>Consideración de la carga de trabajo

Los recursos compartidos de archivos premium proporcionan IOPS y una capacidad de rendimiento para responder a las necesidades de múltiples cargas de trabajo. Sin embargo, en el caso de cargas de trabajo con un uso intensivo de E/S, considere la [instancia de clúster de conmutación por error de SQL Server con Espacios de almacenamiento directo](virtual-machines-windows-portal-sql-create-failover-cluster.md) basada en discos o ultradiscos Premium administrados.  

Compruebe la actividad de IOPS de su entorno actual y compruebe que los archivos premium proporcionarán la capacidad de IOPS que necesita antes de iniciar una implementación o migración. Use los contadores de disco del Monitor de rendimiento de Windows y supervise el total de IOPS (transferencias de disco/s) y el rendimiento (bytes de disco/s) necesarios para archivos de datos de SQL Server, registro y bases de datos temporales. Muchas cargas de trabajo tienen ráfagas de E/S, por lo que es una buena idea realizar comprobaciones durante períodos de uso intensivo y anotar tanto el valor IOPS máximo como el valor IOPS medio. Los recursos compartidos de archivos Premium proporcionan IOPS en función del tamaño del recurso compartido. Los archivos Premium también proporcionan ráfagas gratuitas en las que puede aumentar la capacidad de E/S para triplicar la cantidad de base de referencia durante un máximo de una hora. 
...
Los clientes con Contrato Enterprise con Software Assurance tienen derecho a utilizar un nodo FCI pasivo gratuito por cada nodo activo. Para aprovechar esta ventaja en Azure, use imágenes de VM BYOL y, después, use la misma licencia en los nodos activos y pasivos de FCI. Para obtener más información, consulte [Contrato Enterprise](https://www.microsoft.com/Licensing/licensing-programs/enterprise.aspx).
...
1. Si utiliza una de las imágenes de máquina virtual basada en SQL Server, quite la instancia de SQL Server.
   ...
   - En **Programas y características**, haga clic con el botón derecho en **Microsoft SQL Server 64 201_ (64 bits)** y haga clic en **Desinstalar o cambiar**.
   - Haga clic en **Quitar**.
...
## <a name="step-2-mount-premium-file-share"></a>Paso 2: Montaje del recurso compartido de archivos Premium
...
1. Aplique el protocolo de escritorio remoto en la VM con SQL Server con la cuenta que la FCI de SQL Server usará para la cuenta de servicio. 
1. Inicie una consola de comandos de PowerShell de administración. 
1. Ejecute los comandos desde el portal que guardó anteriormente. 
1. Navegue hasta el recurso compartido con el explorador de archivos o el cuadro de diálogo **Ejecutar** (tecla Windows + r) mediante la ruta de acceso de red `\\storageaccountname.file.core.windows.net\filesharename`. Ejemplo: `\\sqlvmstorageaccount.file.core.windows.net\sqlpremiumfileshare`
...
1. Repita estos pasos en cada VM con SQL Server que participará en el clúster. 
  ...
  > [!IMPORTANT]
  > Considere la posibilidad de usar un recurso compartido de archivos independiente para los archivos de copia de seguridad a fin de ahorrar la capacidad de IOPS y tamaño de este recurso compartido para usarla para el archivo de registro y de datos. Puede usar un recurso compartido de archivos Premium o Estándar para los archivos de copia de seguridad

## <a name="step-3-configure-failover-cluster-with-file-share"></a>Paso 3: Configuración del clúster de conmutación por error con el recurso compartido de archivos 

El siguiente paso es configurar el clúster de conmutación por error. En este paso, realizará los siguientes pasos secundarios:
...
1. En **Selección de pruebas**, incluya todas las pruebas, excepto **Almacenamiento** y **Espacios de almacenamiento directo**. Vea la siguiente imagen:

   :::image type="content" source="media/virtual-machines-windows-portal-sql-create-failover-cluster-premium-file-share/cluster-validation.png" alt-text="Pruebas de validación de clústeres":::
...
El siguiente código de PowerShell crea un clúster de conmutación por error para **Windows Server 2012-2016**. Actualice el script con los nombres de los nodos (los nombres de las máquinas virtuales) y una dirección IP disponible desde la red virtual de Azure:
...
Pruebe la conmutación por error del clúster. En Administrador de clústeres de conmutación por error, haga clic con el botón derecho en el clúster > **Más acciones** > **Move Core Cluster Resource** (Mover recurso del clúster del recurso principal) > **Seleccionar nodo** y seleccione el otro nodo del clúster. Mueva el recurso de clúster principal a cada nodo del clúster y, después, devuélvalo al nodo principal. Si puede mover correctamente el clúster a cada nodo, está listo para instalar SQL Server.  
...
## <a name="step-5-create-sql-server-fci"></a>Paso 5: Crear la FCI de SQL Server
...
1. Haga clic en **Nueva instalación de clúster de conmutación por error de SQL Server**. Siga las instrucciones del asistente para instalar la FCI de SQL Server.
   ...
   Es preciso que los directorios de datos de FCI estén en el recurso compartido de archivos premium. Escriba la ruta de acceso completa del recurso compartido,con el formato `\\storageaccountname.file.core.windows.net\filesharename\foldername`. Aparecerá una advertencia que le notificará que ha especificado un servidor de archivos como directorio de datos. Se espera que esto sea así. Asegúrese de que la cuenta en la que se conserva el recurso compartido de archivos es la misma que usa el servicio SQL Server para evitar posibles errores. 
...
## <a name="step-6-create-azure-load-balancer"></a>Paso 6: Creación del equilibrador de carga de Azure
...
## <a name="step-7-configure-cluster-for-probe"></a>Paso 7: Configurar el clúster para el sondeo
