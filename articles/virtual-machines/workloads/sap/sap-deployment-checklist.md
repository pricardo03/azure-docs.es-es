...
1. Documento de diseño de alto nivel: este documento debe contener:
    ...
    4. Arquitectura para conectarse desde un entorno local a Azure de la red. Empiece a familiarizarse con el [plano técnico del centro de datos virtual de Azure](https://docs.microsoft.com/azure/architecture/vdc/).
...
1. Optimice la transferencia de datos a Azure. Depende en gran medida de la transferencia de los casos de cliente a través de [Azure ExpressRoute](https://azure.microsoft.com/services/expressroute/) localmente era más rápido si el circuito de Express tenía suficiente ancho de banda. Con otros clientes, resultó ser más rápido hacerlo por Internet.
...
3. Validación técnica 
   ...
   2. Almacenamiento
      1.  Use [almacenamiento Azure estándar SSD](https://docs.microsoft.com/azure/virtual-machines/windows/disks-types#standard-ssd) como mínimo para las máquinas virtuales que representan las capas de aplicación de SAP y para la implementación de DBMS confidencial de incumplimiento
      2.  Recomendamos no para usar [discos de Azure Standard HDD](https://docs.microsoft.com/azure/virtual-machines/windows/disks-types#standard-hdd) en general
      2.  Use [Azure Premium Storage](https://docs.microsoft.com/azure/virtual-machines/windows/disks-types#premium-ssd) para todas las máquinas virtuales de DBMS que son sensibles al rendimiento de forma remota
   ...
   3. Redes
      1.  Pruebe y evalúe su infraestructura de red virtual y la distribución de las aplicaciones de SAP a través o dentro de las distintas redes virtuales de Azure.
          ...
          6.   Si usas un equilibrador de carga de Azure junto con la comprobación de los sistemas operativos invitado de Linux que el sistema operativo Linux red parámetro **net.ipv4.tcp_timestamps** está establecido en **0**. Tenga en cuenta respecto a las recomendaciones en versiones anteriores de SAP [2382421 #](https://launchpad.support.sap.com/#/notes/2382421). Mientras tanto, la nota de SAP se actualiza para reflejar el hecho de que el parámetro debe establecerse en 0 para trabajar junto con los equilibradores de carga de Azure.
...
6. Pruebas de rendimiento
   ...
   3.  En SAP basado en medidas y seguimiento de SAP, compare las transferencias de datos a través de interfaces al sistema SAP. Centrarse en las interfaces que sepa que la transferencia va entre distintas ubicaciones, como pasar de local a Azure 
...
5.  Después de la implementación de la infraestructura y la aplicación, pase por una serie de comprobaciones con el fin de validar lo siguiente:
    ...
    14. Pruebe y evalúe la latencia de red entre las máquinas virtuales de nivel de aplicación de SAP y las máquinas virtuales de DBMS según la nota de soporte técnico de SAP [#500235](https://launchpad.support.sap.com/#/notes/500235) y la nota de soporte técnico de SAP [#1100926](https://launchpad.support.sap.com/#/notes/1100926/E). Evalúe los resultados con las instrucciones de latencia de red de la nota de soporte técnico de SAP [#1100926](https://launchpad.support.sap.com/#/notes/1100926/E). La latencia de red debe estar en el intervalo de moderada y buena. Se aplican excepciones al tráfico entre máquinas virtuales y las unidades de instancias grandes de HANA, tal y como se describe [aquí](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-network-architecture#networking-architecture-for-hana-large-instance).
