...
UltraSSD ofrece la posibilidad de definir un único disco que cumpla el intervalo de rendimiento de tamaño, IOPS y disco. En lugar de usar los administradores de volúmenes lógicos como LVM o MDADM sobre Azure Premium Storage para construir los volúmenes que cumplan los requisitos de rendimiento de almacenamiento y e/s por segundo. Puede ejecutar una combinación de configuración entre UltraSSD y Premium Storage. Como resultado puede restringir el uso de UltraSSD para el rendimiento crítico/Hana/Data y/Hana/log volúmenes y cubren los otros volúmenes con Azure Premium Storage
...
Los valores solo se consideran un punto de partida y necesitan evaluar según la demanda real. La ventaja con SSD Ultra Azure es que los valores de IOPS y rendimiento se pueden adaptados sin necesidad de apagar la máquina virtual o detener la carga de trabajo se aplica al sistema.   
